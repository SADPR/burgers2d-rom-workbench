...
def plot_cluster_snapshot_counts(cluster_sizes, out_png):
    ...
    plt.title("Local POD-GPR Stage3 cluster snapshot counts")
    plt.xticks(xs)
...
def main(
    pod_file=os.path.join(script_dir, "local_gpr_pod_per_cluster.npz"),
    snap_folder=os.path.join(parent_dir, "Results", "param_snaps"),
    output_file=os.path.join(script_dir, "local_gpr_q_per_cluster.npz"),
