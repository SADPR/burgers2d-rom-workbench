...
"""
STAGE 2: PROJECT SNAPSHOTS ONTO POD BASIS (POD-RBF)
...
This stage should be run once after stage1 for a chosen basis/primary split.
Then stage3 can be rerun multiple times without recomputing stage1/stage2.
"""
...
def main(
    ...
    if total_modes > n_basis:
        raise ValueError(
            f"Requested total_modes={total_modes}, but basis has {n_basis} columns."
        )
    if primary_modes < 1:
        raise ValueError("primary_modes must be >= 1.")
    if primary_modes >= total_modes:
        raise ValueError(
            f"primary_modes={primary_modes} must be < total_modes={total_modes}."
        )
