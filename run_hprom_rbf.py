...
"""
Run global POD-RBF HPROM (ECSW-LSPG) for the 2D inviscid Burgers problem
...
from burgers.pod_rbf_manifold import (
    ...
    inviscid_burgers_implicit2D_LSPG_pod_rbf_ecsw,
)
...
def main(
    ...
    if weights_file is None:
        weights_file = os.path.join(model_dir, "ecsw_weights_rbf.npy")
    legacy_weights_file = os.path.join(model_dir, "ecm_weights_rbf_global.npy")
    ...
    print(f"[HPROM-RBF] Loaded model from: {model_dir}")
    print(
        f"[HPROM-RBF] U_p shape={model['U_p'].shape}, U_s shape={model['U_s'].shape}, "
        f"q_p_train shape={model['q_p_train'].shape}, q_s_train shape={model['q_s_train'].shape}"
    )
    print(
        f"[HPROM-RBF] kernel={model['kernel_name']}, epsilon={model['epsilon']:.6e}, "
    ...
    if compute_ecsw:
        ...
        print(
            "[HPROM-RBF] ECSW snapshot selection mode="
            f"{ecsw_plan['mode']}, selected {ecsw_plan['num_selected_total']} / "
            f"{ecsw_plan['num_candidates_total']} candidate pairs."
        )
        print(f"[HPROM-RBF] Selected snapshots per mu: {ecsw_plan['num_selected_per_mu']}")
        ...
        for imu, mu_train in enumerate(mu_samples):
            ...
            print(f"[HPROM-RBF] Generating ECSW training block for mu={mu_train}")
        ...
        c = np.vstack(clist)
        C_shape = c.shape
        print(f"[HPROM-RBF] Stacked ECSW training matrix C shape: {C_shape}")
        ...
        print(f"[HPROM-RBF] ECSW weights saved to: {weights_file}")
        print(f"[HPROM-RBF] ECSW solve time: {elapsed_ecsw:.3e} seconds")
        print(f"[HPROM-RBF] ECSW residual: {ecsw_residual:.3e}")

        reduced_mesh_plot_path = os.path.join(results_dir, "hprom_rbf_reduced_mesh.png")
        plt.figure(figsize=(7, 6))
        plt.spy(weights.reshape((num_cells_y, num_cells_x)))
        plt.xlabel(r"$x$ cell index")
        plt.ylabel(r"$y$ cell index")
        plt.title("HPROM-RBF Reduced Mesh (ECSW)")
        plt.tight_layout()
        plt.savefig(reduced_mesh_plot_path, dpi=300, bbox_inches="tight")
        plt.close()
        print(f"[HPROM-RBF] Reduced mesh plot saved to: {reduced_mesh_plot_path}")
    else:
        if os.path.exists(weights_file):
            weights = np.asarray(np.load(weights_file, allow_pickle=False), dtype=np.float64)
            weights_source = "loaded"
            print(f"[HPROM-RBF] Loaded ECSW weights from: {weights_file}")
            ...
            weights_source = "loaded_legacy"
            print(f"[HPROM-RBF] Loaded legacy ECSW weights from: {legacy_weights_file}")
        else:
            raise FileNotFoundError(
    ...
    print(f"[HPROM-RBF] N_e (nonzero ECSW weights): {n_ecsw_elements}")

    t0 = time.time()
    red_coords, hprom_stats = inviscid_burgers_implicit2D_LSPG_pod_rbf_ecsw(
        grid_x=grid_x,
        grid_y=grid_y,
        w0=w0,
        dt=dt,
    ...
    print(f"[HPROM-RBF] Elapsed HPROM time: {elapsed_hprom:.3e} seconds")
    print(f"[HPROM-RBF] Gauss-Newton iterations: {num_its}")
    print(
        "[HPROM-RBF] Timing breakdown (s): "
    ...
    print(f"[HPROM-RBF] Elapsed HDM load/solve time: {elapsed_hdm:.3e} seconds")

    rom_path = os.path.join(
        results_dir,
        f"hprom_rbf_snaps_mu1_{mu1:.2f}_mu2_{mu2:.3f}.npy",
    )
    np.save(rom_path, rom_snaps)
    print(f"[HPROM-RBF] HPROM snapshots saved to: {rom_path}")
    ...
    fig_path = os.path.join(
        results_dir,
        f"hprom_rbf_mu1_{mu1:.2f}_mu2_{mu2:.3f}.png",
    )
    plt.savefig(fig_path, dpi=300, bbox_inches="tight")
    plt.close(fig)
    print(f"[HPROM-RBF] Comparison plot saved to: {fig_path}")
    ...
    print(f"[HPROM-RBF] Relative error: {relative_error:.2f}%")

    report_path = os.path.join(
        results_dir,
        f"hprom_rbf_summary_mu1_{mu1:.2f}_mu2_{mu2:.3f}.txt",
    )
    write_txt_report(
        report_path,
        [
    ...
    print(f"[HPROM-RBF] Text summary saved to: {report_path}")
