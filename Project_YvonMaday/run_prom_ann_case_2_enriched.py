...
def _load_or_build_case2_ecsw_weights(
    ...
    weights_path = os.path.join(
        weights_dir,
        f"ecsw_weights_ann_case2_n{n_primary}_ntot{total_modes}{suffix}.npy",
    )
...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Run enriched Case 2 ANN closure with PROM/HPROM backend."
    )
    ...
    run_tag = f"case2_{backend_tag}_ann_enriched_{tag}_n{n_p}_ntot{total_modes}"
