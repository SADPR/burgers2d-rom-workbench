...
"""
Run Case 2 GPR closure ROM with selectable backend.
...
def _load_or_build_case2_ecsw_weights(
    ...
    weights_path = os.path.join(
        RUNS_ECSW_DIR,
        f"ecsw_weights_gpr_case2_n{n_primary}_ntot{total_modes}.npy",
    )
...
def _load_case2_model(model_path, device):
    ...
    if str(ckpt.get("format", "")).strip().lower() != "gpr_map":
        raise ValueError(
            f"{model_path} is not a GPR-map checkpoint. "
            "Train with stage3_perform_training_case_2_gpr.py first."
        )
    n_s = int(ckpt.get("out_dim", ckpt.get("n_s")))
    in_dim = int(ckpt.get("in_dim", 3))
    if in_dim != 3:
        raise ValueError(f"Case2 GPR checkpoint in_dim={in_dim}, expected 3")
...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Run Case 2 GPR closure with PROM/HPROM backend."
    )
    ...
    gpr_model, n_s, ckpt = _load_case2_model(model_path, device=device)
    gpr_model.eval()
    ...
    run_tag = f"case2_{backend_tag}_gpr_{tag}_n{n_p}_ntot{total_modes}"
    ...
    plot_snaps(
        ...
        rom_snaps,
        plot_steps,
        label="HPROM-GPR Case 2" if effective_backend == "hprom" else "PROM-GPR Case 2",
        fig_ax=(fig, ax1, ax2),
        color="blue",
        linewidth=1.8,
        linestyle="solid",
