...
"""
Run Case 1 RBF closure ROM with selectable backend.
...
- ECSW weights are case-specific for HPROM-RBF Case 1.
"""
...
class RBFMuCompatWrapper(nn.Module):
    """
    ECSW case1 manifold currently calls map on [q_p, mu].
    Keep only q_p for Case 1 RBF closure.
    """
    ...
    def forward(self, x):
        ...
        if x.size(0) < self.n_primary:
            raise ValueError(
                f"RBF input size {x.size(0)} smaller than n_primary={self.n_primary}."
            )
...
def _load_or_build_case1_ecsw_weights(
    ...
    weights_path = os.path.join(
        RUNS_ECSW_DIR,
        f"ecsw_weights_rbf_case1_n{n_primary}_ntot{total_modes}.npy",
    )
...
def _load_case1_model(model_path, device):
    ...
    if str(ckpt.get("format", "")).strip().lower() != "rbf_map":
        raise ValueError(
            ...
            "Train with stage3_perform_training_case_1_rbf.py first."
        )
    n_p = int(ckpt["in_dim"])
...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Run Case 1 RBF closure with PROM/HPROM backend."
    )
    ...
    if effective_backend == "hprom":
        ...
        ann_ecsw = RBFMuCompatWrapper(rbf_model, n_p).to(device)
        ann_ecsw.eval()
    ...
    run_tag = f"case1_{backend_tag}_rbf_{tag}_n{n_p}_ntot{total_modes}"
    ...
    plot_snaps(
        ...
        rom_snaps,
        plot_steps,
        label="HPROM-RBF Case 1" if effective_backend == "hprom" else "PROM-RBF Case 1",
        fig_ax=(fig, ax1, ax2),
        color="blue",
        linewidth=1.8,
        linestyle="solid",
