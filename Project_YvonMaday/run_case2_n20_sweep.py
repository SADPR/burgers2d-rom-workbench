...
"""
Run a robust Case-2 n=20 sweep:
  1) Train multiple ANN configurations/seeds.
  2) Evaluate offline map error on verification + test points.
  3) Produce a weighted ranking.

This avoids backend mismatch surprises by allowing an explicit dataset directory.
"""
...
def _validate_dataset_dir(raw_path: str, expected_backend: str) -> str:
    ...
    if len(dataset_dir) == 0:
        raise ValueError(
            "--dataset-dir was provided but is empty. "
            "This usually means your shell variable resolved to an empty string."
        )
    ...
    if backend != wanted:
        raise ValueError(
            f"--dataset-dir backend mismatch: solve_backend='{backend}', expected '{wanted}'. "
            f"dataset_dir={d}"
        )
    return str(d)
...
def main(argv=None) -> None:
    parser = argparse.ArgumentParser(description="Case-2 n=20 sweep runner (train + offline ranking).")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional explicit Stage2 dataset directory (contains per_mu/ and meta.npy).",
    )
    parser.add_argument("--primary-modes", type=int, default=20)
    parser.add_argument("--seeds", type=str, default="11,23,42,77")
    parser.add_argument("--model-prefix", type=str, default="case2_model_n20")
    parser.add_argument("--reference-source", choices=("linear_runs", "stage2"), default="linear_runs")
    parser.add_argument("--output-dir", type=str, default="Figures/offline_case2")
    parser.add_argument("--python", type=str, default="python3")
    parser.add_argument(
        "--unbuffered",
        action="store_true",
        help="Force unbuffered Python output for child commands (live progress).",
    )
    parser.add_argument("--skip-train", action="store_true", help="Skip training and only run evaluation/ranking.")
    parser.add_argument(
        "--score-weights",
        type=str,
        default="0.2,0.4,0.4",
        help="Weights for (verification, test1, test2).",
    )
    args = parser.parse_args(argv)

    root = Path(__file__).resolve().parent
    train_script = root / "stage3_perform_training_case_2_ann_test_n20.py"
    ...
    if len(weights) != 3:
        raise ValueError("--score-weights must have exactly three comma-separated values.")
    ...
    for model_name in model_names:
        p = root / "Results" / "Stage3" / "models" / model_name
        if p.exists():
            model_paths.append(p)
        else:
            print(f"[warn] missing model, skipping: {p}")
    if not model_paths:
        raise RuntimeError("No sweep model checkpoints were found.")
