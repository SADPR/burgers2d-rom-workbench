...
"""
Run non-intrusive POD-DL-ROM against HDM (enriched model).
...
def _resolve_device(device):
    ...
    if dev == "cuda" and not torch.cuda.is_available():
        print("[POD-DL-Enriched] CUDA requested but unavailable. Falling back to CPU.")
        return "cpu"
...
def _load_pod_dl_model(model_path, device):
    if not os.path.exists(model_path):
        raise FileNotFoundError(
            ...
            "Run stage3_perform_training_pod_dl_data_driven_enriched.py first."
        )
...
def main(
    ...
    model_name="pod_dl_data_driven_model_enriched.pt",
    ...
    ensure_layout_dirs()
    ensure_enrichment_dirs()
    os.makedirs(ENRICHMENT_RUNS_POD_DL_DIR, exist_ok=True)
    ...
    print(f"[POD-DL-Enriched] device = {runtime_device}")
    print(f"[POD-DL-Enriched] model = {model_path}")
    print(f"[POD-DL-Enriched] basis = {basis_path} (available={basis_available}, using={total_modes})")
    print(f"[POD-DL-Enriched] u_ref = {uref_path if os.path.exists(uref_path) else 'zeros'}")
    print(f"[POD-DL-Enriched] model_ntot = {model_ntot}")
    print(f"[POD-DL-Enriched] snap_folder = {snap_folder}")
    ...
    run_tag = f"pod_dl_data_driven_enriched_{tag}_ntot{total_modes}_nz{z_pred.shape[0]}"
    out_dir = os.path.join(ENRICHMENT_RUNS_POD_DL_DIR, run_tag)
    ...
    summary_path = os.path.join(out_dir, "pod_dl_data_driven_enriched_summary.txt")
    write_kv_txt(
        summary_path,
        [
            ("mu_test", mu_test),
            ("method", "pod_dl_data_driven_nonintrusive_enriched"),
    ...
    print(f"[POD-DL-Enriched] inference_time = {infer_elapsed:.3e} s")
    print(f"[POD-DL-Enriched] relative error vs HDM: {rel_err:.2f}%")
    print(f"[POD-DL-Enriched] saved run folder: {out_dir}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run POD-DL non-intrusive ROM (enriched).")
    ...
    parser.add_argument(
        "--model-name",
        type=str,
        default="pod_dl_data_driven_model_enriched.pt",
        help="Checkpoint filename under Results_Enrichment/Stage3/<dataset>/models (used if --model-path is not set).",
    )
    parser.add_argument(
        "--model-path",
        type=str,
