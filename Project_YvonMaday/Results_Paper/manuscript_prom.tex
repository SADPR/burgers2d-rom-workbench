...
Two additional trajectories are retained as validation data for the parameter--time master map in the enriched
campaign; these validation points are not part of the four test points reported below.  The baseline HPROM training
records use their original 10\% held-out snapshot split, while the enriched master-map record uses these two
separate trajectories.  The four evaluation points are
\begin{align*}
  \mub^{(v)} &= (4.875,0.0225), &
  \mub^{(1)} &= (4.560,0.0190),\\
  \mub^{(2)} &= (5.190,0.0260), &
...
\caption{Effect of HPROM coefficient-data enrichment on state error.  The dashed level is the fixed linear HPROM
