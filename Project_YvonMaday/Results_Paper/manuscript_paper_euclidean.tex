...
projection-based reduced-order models (PROMs) of a nonlinear time-dependent
system.
All models use a common orthonormal proper-orthogonal-decomposition (POD)
coordinate system.  The learned families comprise \textsc{PROM-ANN} closures,
an intrusive \textsc{PROM-POD-AE} decoder manifold, and the nonintrusive
...
The parameter--time closure of Case~2 is especially attractive: it avoids
requiring a single-valued primary-to-secondary relation across trajectories,
and its iterate-independent trial tangent makes the online residual solve
comparatively simple.  Yet its prescribed secondary coordinates cannot
respond to the current residual.  We augment that tail with a small
residual-adaptive space obtained by eliminating the primary increment in the
linearized
...
\end{equation}
Here $r$ is a user-selected computational budget.  The construction uses
orthogonalized operator applications, so it does not explicitly form powers
of $\mathbf H$.
...
\end{equation}
One way to obtain the orthonormal columns of $\mathbf B_r$ is Arnoldi
orthogonalization of the operator actions in \eqref{eq:adaptive-actions}.
If $\mathbf g$
...
Algorithm~\ref{alg:case2-online} summarizes the complete online
advance for both full and weighted sampled residuals.  A new correction
space is constructed at each time step and then frozen during its nonlinear
solve.  Its amplitudes start at zero because the columns generally differ
between steps; the previous correction nevertheless enters the next
time-discrete residual through the computed predecessor state.
...
\begin{equation}
\left\{\utilde_{\bar n}^{m+1}(\q,\mathbf a;\mub)\right\}
...
\begin{equation}
\min_{\Delta\mathbf y_r\in\mathbb R^{n+r_m}}
\norm{\mathbf S_\xi\mathbf r+
\mathbf S_\xi\mathbf J
[\mathbf V\quad\overline{\mathbf V}\mathbf B_{r,\xi}]
\Delta\mathbf y_r}_2^2,
\qquad \mathbf y_r=\begin{bmatrix}\q\\\mathbf a\end{bmatrix}.
...
For Algorithm~\ref{alg:case2-online}, take $\mathbf S=\mathbf I_N$ for a
full-residual PROM or $\mathbf S=\mathbf S_\xi$ for an HPROM.  The HPROM
uses its fixed, model-specific positive rule for both direction selection
and the nonlinear solve.  The branch $r=0$ recovers uncorrected Case~2
under the same rule, initialization, and solver settings.
...
\For{$m=0,\ldots,N_t-1$}
  ...
  \If{$r>0$}
    ...
    \State Orthonormalize $\mathcal K_r(\mathbf H,\mathbf g)$ to obtain
    $\mathbf B^{m+1}$, stopping at breakdown
  \Else
  ...
  \While{the Gauss--Newton stopping criterion is not met}
    ...
    \State Compute $\Delta\mathbf y_r^{m+1,k}$ satisfying
    \Statex \hspace{\algorithmicindent}$
    \widehat{\mathbf W}_k^T\widehat{\mathbf W}_k
    \Delta\mathbf y_r^{m+1,k}
    =-\widehat{\mathbf W}_k^T\widehat{\mathbf r}^{m+1,k}$
...
The system displayed in Algorithm~\ref{alg:case2-online} is the normal-equation
condition for the least-squares increment in
\eqref{eq:adaptive-gn} or \eqref{eq:adaptive-sampled-gn}; it need not be
assembled or solved as a Gram system.  The reported computations solve the
linear least-squares problem directly and select the minimum-norm increment
if the Jacobian--tangent product is rank deficient.
...
its off-diagonal entries by $\sqrt{2}$, so its vector norm equals the
...
\end{equation}
This is a cubature consistency condition in the fixed common basis, not a
change of trial coordinates.
...
\label{sec:benchmark}
\subsection{Burgers problem and fixed common basis}
...
POD basis.  Its retained 151 modes capture $99.9900\%$ of the
...
family, including the direct nonintrusive maps.  This is an experimental
control linking their labels to a reduced dynamical solve, not a claim that
they become intrusive models.
More precisely, for training parameters $\{\mub_j\}_{j=1}^{N_\mu}$ the
regression data are
\begin{equation}
\mathcal D_N^{\mathrm H}=
\left\{\left(\mub_j,t^m,\q_N^m(\mub_j)\right):
j=1,\ldots,N_\mu,\ m=0,\ldots,N_t\right\},
\label{eq:hprom-coordinate-training-data}
\end{equation}
where every $\q_N^m(\mub_j)$ is obtained by advancing the linear HPROM with
\eqref{eq:linear-hprom-gn-update}.  The training labels are therefore
the particular realization of $\qtot$ generated by that reduced solve;
they are not constructed as orthogonal projections
$\mathbf V_{\mathrm{tot}}^T(\uvec^m(\mub_j)-\uvec_{\mathrm{ref}})$ of
HDM snapshots.
Cases~1--3 use the blocks in
\eqref{eq:linear-hprom-reference-partition}; the autoencoder and both direct
maps use the complete $\q_N$.  This is the precise sense in which the main
learned representations are linear-HPROM-consistent.
...
only; a $9+36$ campaign is not part of the reported study.
...
Table~\ref{tab:euclidean-enrichment-dataset} summarizes the resulting
trajectory and parameter counts, while Fig.~\ref{fig:parameter-space} shows
the nested spatial coverage of the parameter domain.
...
$1$, $0.03$, and $0.01$.  All use the same fixed POD coordinate system.
...
The controlled parameter--time master map predicts the entire reference
coordinate vector,
\begin{equation}
...
\label{eq:master-map}
\end{equation}
Here the hat distinguishes the network prediction $\widehat{\q}_N$ from its
linear-HPROM target $\q_N$.
...
Figure~\ref{fig:shared-map-deployments} summarizes the three deployments and
identifies which coordinates remain residual-solved in each one.
...
\begin{tabular}{l|c|c}
Deployment & Complete deployed coordinate $\qtot$ & Residual-solved unknowns\\
\hline
...
Table~\ref{tab:hprom-cubature} shows that each conventional nonlinear ECM
rule retains 901 cells, compared with 5621 for the frozen linear-HPROM
rule.  The separate B3 moment fit retains 1506--1534 cells across the
reported data budgets.  A smaller sampled support does not by itself
determine the total online time.
...
The reported Krylov bases use Arnoldi orthogonalization with two
reorthogonalization passes for each direction after the first.
...
For every reporting parameter $\mub$, denote the complete coordinate
output of a model by $\mathbf q_{\mathrm{tot}}^{\mathrm{ROM},m}$ and its
state reconstruction by
$\utilde_{\mathrm{ROM}}^m=\uvec_{\mathrm{ref}}+
\mathbf V_{\mathrm{tot}}\mathbf q_{\mathrm{tot}}^{\mathrm{ROM},m}$.
Collect the state histories and the reference and model coordinate
histories as
\begin{equation}
\begin{aligned}
\mathbf U(\mub)&=
\begin{bmatrix}\uvec^0&\cdots&\uvec^{N_t}\end{bmatrix},\\
\widetilde{\mathbf U}^{\mathrm{ROM}}(\mub)&=
\begin{bmatrix}\utilde_{\mathrm{ROM}}^0&\cdots&
\utilde_{\mathrm{ROM}}^{N_t}\end{bmatrix},\\
\mathbf Q_N(\mub)&=
\begin{bmatrix}\q_N^0&\cdots&\q_N^{N_t}\end{bmatrix},\\
\mathbf Q_{\mathrm{tot}}^{\mathrm{ROM}}(\mub)&=
\begin{bmatrix}\mathbf q_{\mathrm{tot}}^{\mathrm{ROM},0}&\cdots&
\mathbf q_{\mathrm{tot}}^{\mathrm{ROM},N_t}\end{bmatrix}.
\end{aligned}
\label{eq:state-coordinate-histories}
\end{equation}
Every column is evaluated at the same $\mub$.
The state matrices have size $N\times(N_t+1)$, whereas the coordinate
matrices have size $n_{\mathrm{tot}}\times(N_t+1)$.
The reported trajectory errors are
\begin{align}
e_u(\mub)&=100\frac{\norm{\mathbf U-
\widetilde{\mathbf U}^{\mathrm{ROM}}}_F}
{\norm{\mathbf U}_F},\label{eq:state-error}\\
e_q(\mub)&=100\frac{\norm{\mathbf Q_N-
...
always the frozen full-coordinate \emph{linear HPROM}, even for nonintrusive
...
$\norm{q_{N,i}-q_{\mathrm{tot},i}^{\mathrm{ROM}}}_2$ over time; relative
plots divide by $\norm{q_{N,i}}_2$ and multiply by 100.
...
and the two nonintrusive maps.  This order is retained in every subsequent
...
Table~\ref{tab:online-coefficients} reports the complete-coordinate errors
behind the state-level comparison, using the frozen linear HPROM as teacher.
\begin{table}[H]
\centering
...
amplitudes while the reconstructed $\mathbf q_{\mathrm{tot},r}$ still has
151 entries.
...
Figure~\ref{fig:hprom-b3-histories} resolves the aggregate HPROM errors in
time and also compares B3 across all four training-data budgets.
...
Table~\ref{tab:hprom-b3-budget} reports the retained support, measured cost,
and accuracy of the accepted 2048-draw rule and its 4096-draw comparator.
...
Table~\ref{tab:hprom-enrichment-training} separates the effect of this data
ladder on training and held-out regression errors before online deployment.
...
Table~\ref{tab:hprom-enrichment-coefficients} and
Fig.~\ref{fig:hprom-enrichment-coefficients} compare complete-coordinate
recovery across the same budgets used in the state-error study.
\begin{table}[H]
\centering
...
Figures~\ref{fig:hprom-enriched-solutions} and
\ref{fig:hprom-enriched-coefficients} show the corresponding $9+18$ solution
cuts and per-coordinate trajectories under the common plotting limits.
...
Fig.~\ref{fig:coefficient-curves}.  All models, including both nonintrusive
...
model-specific positive cubature in a fixed common POD basis.
...
Coefficient errors use \eqref{eq:coefficient-error} with this PROM
reference, rather than the linear HPROM used in the main experiments.
...
\begin{equation}
\utilde_d^{\mathrm{C2}}(\mathbf x;\mub,t)
=\uvec_{\mathrm{ref}}+\mathbf V_{1:d}\mathbf x+
\mathbf V_{d+1:151}
\bigl[\mathcal G_q^{\mathrm{PROM}}(\mub,t)\bigr]_{d+1:151},
...
Table~\ref{tab:adaptive-rank-validation} reports the resulting
equal-dimensional trajectory errors, and
Fig.~\ref{fig:adaptive-rank-validation} compares their rank dependence with
the corresponding converged-residual reduction.
...
Figure~\ref{fig:adaptive-state-histories} shows how these aggregate
state-history errors accumulate over time at each reporting parameter.
...
and Figs.~\ref{fig:case2-sweep-state} and
\ref{fig:case2-sweep-coefficients} show that increasing $n$ reduces state and
coordinate errors, with the sharpest state improvement between $n=50$ and
...
Tables~\ref{tab:hprom-lhs8-state} and
\ref{tab:hprom-lhs8-coefficients} give the pointwise state and coordinate
errors.  Figures~\ref{fig:hprom-lhs8-solutions} and
\ref{fig:hprom-lhs8-coefficients} provide their solution-cut and
per-coordinate counterparts.
\begin{table}[H]
\centering
...
Tables~\ref{tab:hprom-lhs12-state} and
\ref{tab:hprom-lhs12-coefficients} give the pointwise state and coordinate
errors.  Figures~\ref{fig:hprom-lhs12-solutions} and
\ref{fig:hprom-lhs12-coefficients} show the matching solution cuts and
per-coordinate histories.
\begin{table}[H]
\centering
...
Tables~\ref{tab:hprom-lhs18-state} and
\ref{tab:hprom-lhs18-coefficients} complete the pointwise tabulation for the
largest data budget; its solution and coefficient plots appear in
Figs.~\ref{fig:hprom-enriched-solutions} and
\ref{fig:hprom-enriched-coefficients}.
\begin{table}[H]
\centering
...
\end{table}
The absolute and relative coefficient-error histories are shown for the
baseline campaign in Figs.~\ref{fig:hprom-baseline-heat-absolute} and
\ref{fig:hprom-baseline-heat-relative}, for $9+8$ in
Figs.~\ref{fig:hprom-lhs8-heat-absolute} and
\ref{fig:hprom-lhs8-heat-relative}, for $9+12$ in
Figs.~\ref{fig:hprom-lhs12-heat-absolute} and
\ref{fig:hprom-lhs12-heat-relative}, and for $9+18$ in
Figs.~\ref{fig:hprom-lhs18-heat-absolute} and
\ref{fig:hprom-lhs18-heat-relative}.  This ordered sequence uses the common
colour scales described above so that changes across budgets remain
comparable.
\begin{figure}[H]
\centering
