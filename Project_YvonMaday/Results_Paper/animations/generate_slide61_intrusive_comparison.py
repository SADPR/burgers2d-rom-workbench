#!/usr/bin/env python3
"""Generate the Slide 61 four-model comparison at mu=(5.19, 0.026)."""
...
PREVIEW = HERE / "previews" / "slide61_intrusive_comparison_t12p5.png"
OUTPUT = HERE / "outputs" / "slide61_intrusive_comparison_mu2.gif"
...
def formula_label(model: str) -> str:
    formulas = {
        "Linear HPROM": (
            r"$\tilde{\mathbf u}=\mathbf u_{\mathrm{ref}}+\mathbf V_{\mathrm{tot}}\mathbf q_{\mathrm{tot}}$"
        ),
        "PROM-ANN Case 1": (
...
def load_comparison_data() -> dict[str, tuple[np.ndarray, np.ndarray]]:
    basis = assets.load_npy(assets.METRIC / "basis.npy")
    u_ref = np.asarray(
        assets.load_npy(assets.METRIC / "u_ref.npy"),
        dtype=float,
    ).reshape(-1)
    q_linear = np.asarray(assets.load_npy(assets.linear_q_path(POINT)), dtype=float)

    vx = np.asarray(basis[assets.IDX_X_CUT, : assets.NTOT], dtype=float)
    vy = np.asarray(basis[assets.IDX_Y_CUT, : assets.NTOT], dtype=float)
    linear = (
        u_ref[assets.IDX_X_CUT, None] + vx @ q_linear,
        u_ref[assets.IDX_Y_CUT, None] + vy @ q_linear,
    )
...
def create_figure(
    ...
    comparisons = ("Linear HPROM", "PROM-ANN Case 1", "PROM-POD-AE")
    ...
    styles = {
        ...
        "Linear HPROM": {
            "color": assets.COLORS["Linear HPROM"],
            "linewidth": 2.3,
            "linestyle": "-",
            "alpha": 0.90,
            "zorder": 3,
