...
export LOG_DIR="$PAPER_ROOT/logs/master_dense_gpr_4pts"
...
DENSE_GPR_MODEL_PATH="${DENSE_GPR_MODEL_PATH:-$PAPER_ROOT/Stage3/models/master_dense_gpr_mu_t_to_qtot_ntot151_${DENSE_GPR_LABEL}.pt}"
...
DATA_ROOT="$PAPER_ROOT/Runs/DataDriven_MasterDenseGPR"
CASE2_ROOT="$PAPER_ROOT/Runs/PROM/Case2_MasterDenseGPR_NSweep/np${N_PRIMARY}"

mkdir -p "$MPLCONFIGDIR" "$LOG_DIR" "$DATA_ROOT" "$CASE2_ROOT"

if [[ ! -f "$DENSE_GPR_MODEL_PATH" ]]; then
  echo "Missing dense-GPR checkpoint: $DENSE_GPR_MODEL_PATH" >&2
  echo "Run Results_Paper/scripts/run_mlspg_prom_main_train_master_dense_gpr.sh first." >&2
  exit 1
fi
...
echo "[master-dense-gpr-4pts] family:    $FAMILY"
echo "[master-dense-gpr-4pts] model:     $DENSE_GPR_MODEL_PATH"
echo "[master-dense-gpr-4pts] basis:     $BASIS_PATH"
echo "[master-dense-gpr-4pts] u_ref:     $U_REF_PATH"
echo "[master-dense-gpr-4pts] data root: $DATA_ROOT"
echo "[master-dense-gpr-4pts] case2 root:$CASE2_ROOT"
echo "[master-dense-gpr-4pts] n primary: $N_PRIMARY"
echo "[master-dense-gpr-4pts] device:    $ONLINE_DEVICE"
echo "[master-dense-gpr-4pts] threads:   $PROM_NUM_THREADS"
echo "[master-dense-gpr-4pts] force:     $FORCE"
...
run_point() {
  ...
  if [[ "$FAMILY" == "all" || "$FAMILY" == "data" ]]; then
    if [[ "$FORCE" != "1" && -f "$data_summary" ]]; then
      echo "[skip] data-driven dense-GPR exists: $data_summary"
    else
      echo "[run] Data-driven dense-GPR: mu=(${mu1},${mu2})"
      python3 -u run_rom_data_driven.py \
        --mu1 "$mu1" \
        --mu2 "$mu2" \
        --total-modes 151 \
        --device "$ONLINE_DEVICE" \
        --model-path "$DENSE_GPR_MODEL_PATH" \
  ...
  local case2_summary="$CASE2_ROOT/case2_prom_ann_master_dense_gpr_mu1_${mu1}_mu2_${mu2}_n${N_PRIMARY}_ntot151_summary.txt"
  if [[ "$FAMILY" == "all" || "$FAMILY" == "case2" ]]; then
    if [[ "$FORCE" != "1" && -f "$case2_summary" ]]; then
      echo "[skip] Case 2 PROM dense-GPR exists: $case2_summary"
    else
      echo "[run] Case 2 PROM dense-GPR: mu=(${mu1},${mu2}), n=${N_PRIMARY}"
      python3 -u run_prom_ann_case_2.py \
        ...
        --run-tag-extra master_dense_gpr \
        --max-its 20 \
        --relnorm-cutoff 1e-5 \
        --min-delta 1e-2 \
        --linear-solver lstsq \
