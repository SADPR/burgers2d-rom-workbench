...
export LOG_DIR="$PAPER_ROOT/logs/case1_arch_sweep"
...
cleanup_failed_candidates() {
  ...
  if [[ $status -ne 0 ]]; then
    rm -f \
      "$MODELS_DIR"/case1_ann_ntot151_C*.pt \
      "$STAGE3_DIR"/case1_ann_ntot151_C*_summary.txt
    echo "[case1-sweep] Failed; incomplete candidate checkpoints were removed." >&2
  fi
  exit "$status"
}
...
print("[case1-sweep-check] dataset_dir:", p)
print("[case1-sweep-check] metadata:", meta_path)
print("[case1-sweep-check] solve_backend:", meta.get("solve_backend"))
print("[case1-sweep-check] total_modes:", meta.get("total_modes"))
print("[case1-sweep-check] basis_path:", meta.get("basis_path"))
if str(meta.get("solve_backend")).lower() != "hprom":
    raise SystemExit("Case-1 architecture sweep requires the HPROM dataset.")
if int(meta.get("total_modes")) != 151:
    raise SystemExit("Expected total_modes=151.")
PY

# Remove only stale Case-1 artifacts from this paper campaign.
rm -f \
  "$MODELS_DIR"/case1_ann_ntot151_C*.pt \
  "$MODELS_DIR"/case1_ann_ntot151_best.pt \
  "$MODELS_DIR"/case1_hprom_ann_n10_ntot151.pt \
  "$STAGE3_DIR"/case1_ann_ntot151_C*_summary.txt \
  "$STAGE3_DIR"/case1_ann_ntot151_best_summary.txt \
  "$STAGE3_DIR"/case1_hprom_ann_n10_ntot151_summary.txt \
  "$LOG_DIR"/*.log \
...
run_cfg() {
  ...
  echo "==== Case-1 architecture sweep: ${label}"
...
# Compact controlled sweep for q_p in R^10 -> q_s in R^141.
...
for summary in sorted(stage3.glob("case1_ann_ntot151_C*_summary.txt")):
    data = {}
    ...
    label = summary.name.replace("case1_ann_ntot151_", "").replace("_summary.txt", "")
    rows.append(
        {
            "label": label,
            ...
            "candidate_model": str(models / f"case1_ann_ntot151_{label}.pt"),
        }
    )

if not rows:
    raise SystemExit("No Case-1 sweep summaries found.")
...
csv_path = log_dir / "case1_arch_sweep_summary.csv"
with csv_path.open("w", newline="") as f:
    writer = csv.DictWriter(f, fieldnames=list(rows[0]))
    writer.writeheader()
    writer.writerows(rows)

canonical_model = models / "case1_ann_ntot151_best.pt"
canonical_summary = stage3 / "case1_ann_ntot151_best_summary.txt"
canonical_log = log_dir / "case1_ann_ntot151_best.log"

winner_model = models / f"case1_ann_ntot151_{winner_label}.pt"
winner_summary = stage3 / f"case1_ann_ntot151_{winner_label}_summary.txt"
...
text = text.replace(
    f"model_name: case1_ann_ntot151_{winner_label}.pt",
    "model_name: case1_ann_ntot151_best.pt",
)
...
for path in models.glob("case1_ann_ntot151_C*.pt"):
    path.unlink()
for path in stage3.glob("case1_ann_ntot151_C*_summary.txt"):
    path.unlink()
for path in log_dir.glob("C*.log"):
    path.unlink()

print("[case1-sweep-summary]", csv_path)
for row in rows:
    print(
        " ",
        row["label"],
...
print("[case1-sweep-winner]", winner_label)
print("[case1-sweep-model]", canonical_model)
print("[case1-sweep-summary-file]", canonical_summary)
PY

trap - EXIT
echo "[done] Only the selected Case-1 checkpoint is retained:"
echo "       $MODELS_DIR/case1_ann_ntot151_best.pt"
