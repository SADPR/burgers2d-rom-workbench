...
export PAPER_TAG="${PAPER_TAG:-mlspg_prom_enrichment_ext25_lhs36}"
...
[prom-ext25-lhs36-train] selected architectures:
  ...
  PROM-POD-AE:  latent=10, hidden=(512,256,128), GELU, z-score
  POD-DL-ROM:   latent=10, SiLU, low latent regularization
EOF
}
...
if not meta_path.is_file():
    raise SystemExit(f"Missing enriched PROM metadata: {meta_path}\nRun Stage 2 first.")
...
if len(val_dirs) != 2:
    raise SystemExit(f"Expected 2 validation per_mu directories, found {len(val_dirs)} in {val_dataset}.")
...
train_master_ann() {
  local log_dir="$LOG_ROOT/master_ann"
  ...
  local log="$log_dir/master_ann_mu_t_to_qtot_ntot151_best.log"
  local epochs patience
