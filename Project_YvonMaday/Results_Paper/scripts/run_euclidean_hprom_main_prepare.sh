#!/usr/bin/env bash
# Prepare the reproducible Euclidean-POD linear HPROM campaign.
# One fixed linear ECM rule generates the training, validation, and reporting
# coefficient trajectories. It never touches the PROM or MLSG campaigns.
set -euo pipefail
...
STAGE="${1:-all}"
case "$STAGE" in
  all|check|clean|train9|val2|linear4) ;;
  *)
    echo "Usage: $0 [all|check|clean|train9|val2|linear4]" >&2
    exit 2
    ;;
esac

PAPER_ROOT="$PROJECT_DIR/Results_Paper"
PROM_ROOT="${EUCLIDEAN_HPROM_ROOT:-$PAPER_ROOT/euclidean_hprom_main}"
...
ECSW_DIR="$PROM_ROOT/Stage2/ecsw/Linear"
ECSW_WEIGHTS="$ECSW_DIR/ecsw_weights_lspg_ntot151.npy"
LINEAR_RUNS="$PROM_ROOT/Runs/Linear"
...
print(f"[euclidean-hprom-prepare] basis: {basis_path}")
print(f"[euclidean-hprom-prepare] basis shape: {basis.shape}")
print(f"[euclidean-hprom-prepare] u_ref: {uref_path}")
PY
}
...
write_manifest() {
  mkdir -p "$PROM_ROOT"
  cat > "$MANIFEST" <<EOF
campaign: Euclidean POD HPROM
metric: Euclidean POD
basis_path: $BASIS
u_ref_path: $UREF
total_modes: 151
basis_construction: direct SVD; centered snapshots; energy_lost <= 1e-4
solver: sampled-residual LSPG/HPROM
linear_ecm_rule: $ECSW_WEIGHTS
linear_ecm_snapshot_percent: 2.0
linear_ecm_training_parameters: 9
linear_solver: lstsq
normal_eq_reg: 1e-12
max_its: 20
relnorm_cutoff: 1e-5
...
clean_campaign() {
  ...
  if [[ "$PROM_ROOT" != "$PAPER_ROOT/euclidean_hprom_main" ]]; then
    echo "[error] Refusing cleanup outside the default euclidean_hprom_main root: $PROM_ROOT" >&2
    exit 2
  fi
  echo "[clean] Removing only generated Euclidean HPROM campaign outputs under: $PROM_ROOT"
...
dataset_complete() {
  ...
  "$PYTHON_BIN" - "$dataset" "$expected_trajectories" <<'PY'
from pathlib import Path
import sys
import numpy as np
...
for mu_dir in mu_dirs:
    ...
    if qn.shape != (151, 501) or not np.isfinite(qn).all():
        raise SystemExit(f"Invalid qN shape in {mu_dir}: {qn.shape}.")
print(f"[euclidean-hprom-prepare] complete dataset: {dataset} ({len(mu_dirs)} trajectories)")
PY
}

build_train9() {
  if [[ "$FORCE" != "1" && -f "$ECSW_WEIGHTS" ]] && dataset_complete "$TRAIN_DATASET" 9; then
    echo "[skip] Euclidean training dataset is already complete: $TRAIN_DATASET"
    return
  fi
  echo "[run] Building the linear ECM rule and nine HPROM training trajectories."
  set_threads "$PROM_NUM_THREADS"
  "$PYTHON_BIN" -u stage2_build_prom_qn_dataset.py \
    --backend hprom --total-modes 151 \
    --basis-path "$BASIS" --u-ref-path "$UREF" \
    --output-dir "$TRAIN_DATASET" \
    --ecsw-weights-dir "$ECSW_DIR" --rebuild-ecsw \
    --ecsw-snapshot-percent 2.0 --ecsw-num-training-mu 9 \
    --ecsw-snap-time-offset 3 --ecsw-random-seed 42 \
    --ecsw-ensure-mu-coverage --ecsw-svd-rel-tol 1e-8 \
...
build_val2() {
  ...
  require_file "$ECSW_WEIGHTS"
  echo "[run] Building two HPROM external parameter-validation trajectories with the frozen linear rule."
  set_threads "$PROM_NUM_THREADS"
  "$PYTHON_BIN" -u stage2_build_prom_qn_dataset.py \
    --backend hprom --total-modes 151 \
    --basis-path "$BASIS" --u-ref-path "$UREF" \
    --output-dir "$VAL_DATASET" \
    --ecsw-weights-dir "$ECSW_DIR" --no-rebuild-ecsw \
    --ecsw-snapshot-percent 2.0 --ecsw-num-training-mu 9 \
    --ecsw-snap-time-offset 3 --ecsw-random-seed 42 \
    --ecsw-ensure-mu-coverage --ecsw-svd-rel-tol 1e-8 \
...
linear_run_complete() {
  local mu1="$1" mu2="$2"
  local tag
  tag="$(printf 'linear_hprom_mu1_%.3f_mu2_%.4f_ntot151' "$mu1" "$mu2")"
  [[ -f "$LINEAR_RUNS/$tag/summary.txt" && -f "$LINEAR_RUNS/$tag/qN.npy" ]]
}

build_linear4() {
  require_file "$ECSW_WEIGHTS"
  local point mu1 mu2 label mu1_tag mu2_tag
  local points=(
  ...
  for point in "${points[@]}"; do
    ...
    if [[ "$FORCE" != "1" ]] && linear_run_complete "$mu1" "$mu2"; then
      echo "[skip] Linear HPROM $label is complete."
      continue
    fi
    ...
    "$PYTHON_BIN" -u run_prom.py \
      ...
      --basis-path "$BASIS" --u-ref-path "$UREF" \
      --ecsw-weights-path "$ECSW_WEIGHTS" --ecsw-svd-rel-tol 1e-8 \
      --output-root "$LINEAR_RUNS" --no-save-rom-snaps --no-plot \
      2>&1 | tee "$LOG_ROOT/linear_${label}_mu1_${mu1_tag}_mu2_${mu2_tag}.log"
  done
}
...
echo "[euclidean-hprom-prepare] done."
