#!/usr/bin/env bash
# Train the fixed Euclidean-POD HPROM-consistent models with external parameter validation.
...
PAPER_TAG="${PAPER_TAG:-euclidean_hprom_main}"
...
check_dataset() {
  DATASET_DIR="$DATASET_DIR" VALIDATION_DATASET_DIR="$VALIDATION_DATASET_DIR" "$PYTHON_BIN" - <<'PY'
import os
...
for label, dataset, expected_mu in expected:
    ...
    if str(meta.get("solve_backend", "")).lower() != "hprom":
        raise SystemExit(f"{label}: expected solve_backend=hprom, got {meta.get('solve_backend')!r}.")
    ...
    print(f"[euclidean-hprom-train-check] {label}: {dataset}")
    print(f"[euclidean-hprom-train-check] metadata: {meta_path}")
    print(f"[euclidean-hprom-train-check] trajectories: {len(mu_dirs)} x 501")
PY
}
...
train_case1() {
  ...
  echo "==== Train Euclidean HPROM-ANN Case 1 | wide SiLU | n=10"
  run_logged "$model" "$summary" "$log" \
    "$PYTHON_BIN" -u stage3_perform_training_case_1_ann_maday.py \
...
train_master_map() {
  ...
  epochs="$(selected_epochs 6000)"
  patience="$(selected_patience 220)"
  echo "==== Train Euclidean master map for HPROM-ANN Case 2 and POD-NN-ROM | wide SiLU"
  run_logged "$model" "$summary" "$log" \
    "$PYTHON_BIN" -u stage3_perform_training_rom_data_driven_maday.py \
      --maday-results-root "$PAPER_RESULTS_ROOT" --maday-tag "$PAPER_TAG" \
      --dataset-backend hprom --dataset-ntot 151 --dataset-dir "$DATASET_DIR" \
...
train_case3() {
  ...
  epochs="$(selected_epochs 6000)"
  patience="$(selected_patience 220)"
  echo "==== Train Euclidean HPROM-ANN Case 3 | wide SiLU | n=10"
  run_logged "$model" "$summary" "$log" \
    "$PYTHON_BIN" -u stage3_perform_training_case_3_ann_maday.py \
      --maday-results-root "$PAPER_RESULTS_ROOT" --maday-tag "$PAPER_TAG" \
      --dataset-backend hprom --dataset-ntot 151 --dataset-dir "$DATASET_DIR" \
      --validation-dataset-dir "$VALIDATION_DATASET_DIR" --primary-modes 10 \
...
train_pod_ae() {
  ...
  echo "==== Train Euclidean HPROM-POD-AE | latent 10 | GELU | z-score"
  run_logged "$model" "$summary" "$log" \
    "$PYTHON_BIN" -u stage3_perform_training_prom_pod_ae.py \
      --dataset-backend hprom --dataset-ntot 151 --dataset-dir "$DATASET_DIR" \
      --validation-dataset-dir "$VALIDATION_DATASET_DIR" \
...
train_pod_dl() {
  ...
  run_logged "$model" "$summary" "$log" \
    "$PYTHON_BIN" -u stage3_perform_training_pod_dl_data_driven.py \
      --dataset-backend hprom --dataset-ntot 151 --dataset-dir "$DATASET_DIR" \
      --validation-dataset-dir "$VALIDATION_DATASET_DIR" \
      --stage3-dir "$STAGE3_DIR" --models-dir "$MODELS_DIR" \
...
print_plan() {
  cat <<EOF
[euclidean-hprom-train] selected-architecture training with external parameter validation
[euclidean-hprom-train] label:        $CAMPAIGN_LABEL
[euclidean-hprom-train] campaign:     $PAPER_ROOT
[euclidean-hprom-train] training:     $DATASET_DIR ($EXPECTED_TRAIN_TRAJECTORIES trajectories)
[euclidean-hprom-train] validation:   $VALIDATION_DATASET_DIR
[euclidean-hprom-train] stage3:       $STAGE3_DIR
[euclidean-hprom-train] logs:         $LOG_ROOT
[euclidean-hprom-train] family:       $FAMILY
[euclidean-hprom-train] threads:      $TRAIN_NUM_THREADS
[euclidean-hprom-train] force:        $FORCE
[euclidean-hprom-train] validation:   2 external parameter trajectories (1002 rows)
[euclidean-hprom-train] architectures:
  HPROM-ANN Case 1: q_p -> (256,512,512,256) SiLU -> q_s, n=10
  HPROM-ANN Case 2: shared master map (mu,t) -> (256,512,512,256) SiLU -> q_tot
  HPROM-ANN Case 3: [q_p,mu,t] -> (256,512,512,256) SiLU -> q_s, n=10
  HPROM-POD-AE:      151 -> (512,256,128) -> 10 -> mirror, GELU, z-score
...
print_plan
if [[ "$PLAN_ONLY" == "1" ]]; then
  echo "[euclidean-hprom-train] PLAN_ONLY=1; no checks or training were run."
  exit 0
fi

check_dataset
if [[ "$CHECK_ONLY" == "1" ]]; then
  echo "[euclidean-hprom-train] CHECK_ONLY=1; datasets are complete and no training was run."
  exit 0
fi

if [[ "$TRAIN_SMOKE_TEST" == "1" ]]; then
  echo "[euclidean-hprom-train] TRAIN_SMOKE_TEST=1: one epoch, no POD-DL pretraining."
fi
...
echo "[euclidean-hprom-train] completed family=$FAMILY"
