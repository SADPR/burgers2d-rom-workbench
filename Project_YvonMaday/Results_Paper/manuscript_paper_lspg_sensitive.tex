...
compare state-conditioned, parameter--time-conditioned, and hybrid
\textsc{PROM-ANN} closures; an intrusive \textsc{PROM-POD-AE} decoder manifold;
a non-intrusive \textsc{POD-NN-ROM} direct coefficient map; and a
non-intrusive latent \textsc{POD-DL-ROM}. The framework is evaluated on a
parametric two-dimensional inviscid Burgers benchmark using both full-residual
projection and empirical-cubature hyperreduction.
...
coordinate errors into residual-solved coordinates. Linear-PROM and
linear-HPROM trajectories provide the respective campaign learning targets,
which permits data enrichment without acquiring additional high-dimensional
...
\section{Linear PROM/HPROM Reference and Common Basis}
...
\subsection{Basis Metric and Low/High-Coordinate Sensitivity}
\label{sec:metric-aware-basis}

All learned variants below use a common reduced coordinate system.  This
subsection records the basis metric used to construct that system and the
low/high-coordinate diagnostic that motivates the choice.  It is a
basis-selection device, not a separate learning model or an additional
optimization stage.  For the closure constructions introduced later, the
common linear basis is partitioned as
...
construct the basis is part of the common experimental setup, rather than a
model-specific preprocessing choice.

\subsubsection{Euclidean and M-regularized LSPG-sensitive metrics}
...
separately for a particular closure map.  The Burgers-specific sampling and
regularization values are given with the experimental protocol in
Section~\ref{sec:burgers}.

\subsubsection{Low/high transfer diagnostic}
...
\subsubsection{Basis-selection criterion}
...
The PROM--ANN construction considered here is based on neural-network-augmented
projection \cite{barnett2023neural}, in which a nonlinear trial manifold
augments a retained linear subspace with a learned closure for the remaining
reduced coordinates.  In its original state-conditioned form, the learned
relation supplies discarded reduced coordinates while an intrusive projection
solve retains responsibility for the primary block \cite{barnett2023neural}.
This separation is not tied to a neural-network regressor: Gaussian-process
and radial-basis-function closures have been considered for the same latent
closure role \cite{ares2026closure}.  Related assessments of
projection-based reduction for hypersonic-flow benchmarks provide an
application context in which retaining the projection solve is particularly
relevant \cite{chmiel2024assessment,chmiel2025unified}.

Let the common linear trial basis and its
...
The free/dependent coordinate split in \eqref{eq:closure-partition} also
appears in nonlinear compressive reduced-basis theory.  For a solution
manifold parameterized by $p$ physical parameters, the local sensing analysis
identifies $n=p$ as an appropriate local number of free coordinates and
supports a small excess above $p$ over a wider parameter region
\cite{aghili2026nonlinear}.  This result motivates
treating $n$ as a quantity to be assessed rather than fixing it only from POD
energy.  It is not, however, a direct prescription for the transient LSPG
setting below, where the learned relation can also depend on time and the
online dynamics.  In particular, the neural and GPR/RBF closure
constructions above do not select $n$ from the physical parameter dimension;
they prescribe how the dependent coordinates are represented once a split has
been chosen \cite{barnett2023neural,ares2026closure}.
...
The following three choices are information patterns within this common
closure parameterization, not three unrelated nonlinear-manifold methods.
Case~1 is the state-conditioned PROM--ANN closure described above.  Cases~2
and~3 are controlled variants used here to separate a purely
parameter--time-prescribed tail from a tail that can also respond to the
current primary iterate.  The three maps are
\begin{align}
...
$\mathbf u_s^{m_s+1}$ is first represented on the trained manifold.  The
fitted coordinate is
\begin{equation}
\mathbf q_{c,s}
=
...
\end{equation}
For Case~2, this is the linear least-squares problem obtained by subtracting
the prescribed tail,
\begin{equation}
\mathbf q_{2,s}
=
\mathbf V^{\dagger}
\left[
\mathbf u_s^{m_s+1}-\mathbf u_{\mathrm{ref}}
-\overline{\mathbf V}\mathcal M(\bm\mu_s,t_s^{m_s+1})
\right],
\label{eq:ann-ecm-case2-fit}
\end{equation}
where $\dagger$ denotes the Moore--Penrose inverse.  For Cases~1 and~3,
define the state mismatch and its tangent by
\begin{equation}
\boldsymbol\delta_{c,s}(\mathbf y)
=\boldsymbol\phi_c(\mathbf y;\bm\mu_s,t_s^{m_s+1})
-\mathbf u_s^{m_s+1},
\qquad
\mathbf T_{c,s}(\mathbf y)
=\frac{\partial\boldsymbol\phi_c}
{\partial\mathbf y}(\mathbf y;\bm\mu_s,t_s^{m_s+1}),
\qquad c\in\{1,3\}.
\label{eq:ann-ecm-fit-mismatch}
\end{equation}
With $\mathbf P_{\leq n}=[\mathbf I_n\ \mathbf 0]$, we initialize from the
primary block of the full linear least-squares coordinates and apply
Gauss--Newton updates,
\begin{equation}
\mathbf q_{c,s}^{(0)}
=\mathbf P_{\leq n}\mathbf V_{\mathrm{tot}}^{\dagger}
\left(\mathbf u_s^{m_s+1}-\mathbf u_{\mathrm{ref}}\right),
\qquad
\mathbf q_{c,s}^{(k+1)}
=\mathbf q_{c,s}^{(k)}
-\mathbf T_{c,s}\!\left(\mathbf q_{c,s}^{(k)}\right)^{\dagger}
\boldsymbol\delta_{c,s}\!\left(\mathbf q_{c,s}^{(k)}\right).
\label{eq:ann-ecm-nonlinear-fit-update}
\end{equation}
The converged iterate defines $\mathbf q_{c,s}$ in
\eqref{eq:ann-ecm-manifold-fit}; in practice the update is solved as a
least-squares Gauss--Newton step.  Evaluating
\eqref{eq:ann-ecm-projected-residual} at this fitted state gives the entity
contribution
\begin{equation}
\mathbf c_{c,e_i}^{(s)}
=
...
For Cases 1--3, supervised data are extracted from the corresponding linear
solver trajectory $\mathbf q_N=[\mathbf q;\bar{\mathbf q}]$: the linear PROM
for the PROM campaign and the linear HPROM for the HPROM campaign. Case~1 trains $\mathcal N$ from
$(\mathbf q,\bar{\mathbf q})$ and Case~3 trains $\mathcal H$ from
$(\mathbf q,\bm\mu,t,\bar{\mathbf q})$.  The practical Case~2 training
choice used in the reported comparisons is specified in the training protocol
in Sec.~\ref{sec:training-protocol}.
...
Given campaign-consistent linear-solver targets $\mathbf q_N^{\mathrm{ref}}$, the autoencoder
...
\end{equation}
which is also a nonlinear least-squares problem.  With
$\mathbf q_{N,s}^{(0)}=\mathbf V_{\mathrm{tot}}^{\dagger}
(\mathbf u_s^{m_s+1}-\mathbf u_{\mathrm{ref}})$, the encoder supplies the
initial latent coordinate and Gauss--Newton refines it as
\begin{equation}
\mathbf z_s^{(0)}=\mathcal E(\mathbf q_{N,s}^{(0)}),
\qquad
\mathbf z_s^{(k+1)}
=\mathbf z_s^{(k)}
-\mathbf T_{\mathrm{AE}}\!\left(\mathbf z_s^{(k)}\right)^{\dagger}
\left[
\boldsymbol\phi_{\mathrm{AE}}\!\left(\mathbf z_s^{(k)}\right)
-\mathbf u_s^{m_s+1}
\right].
\label{eq:podae-ecm-nonlinear-fit-update}
\end{equation}
The converged iterate is the $\mathbf z_s$ used below. At this fitted latent
state, the POD-AE entity contribution is
\begin{equation}
...
\paragraph{Comparison of intrusive trial maps.}
Table~\ref{tab:intrusive-trial-maps} collects the trial decoder and tangent
basis for every intrusive model considered above.  It makes explicit that the
models differ in their online coordinates and trial maps, not in the governing
time-discrete residual.
\begin{table}[H]
\centering
\caption{Trial decoders and tangent bases of the intrusive models.  The
linear PROM/HPROM row uses the complete linear coordinate vector.  The
remaining rows solve only their displayed online variable.}
\label{tab:intrusive-trial-maps}
\small
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{@{}l c l l@{}}
\toprule
Model & Online variable & Trial decoder $\boldsymbol\phi$ & Tangent $\mathbf T$ \\
\midrule
Linear PROM/HPROM
& $\mathbf q_N$
& $\mathbf u_{\mathrm{ref}}+\mathbf V_{\mathrm{tot}}\mathbf q_N$
...
& $\mathbf q$
& $\mathbf u_{\mathrm{ref}}+\mathbf V\mathbf q+
   \overline{\mathbf V}\mathcal M(\bm\mu,t)$
& $\mathbf V$ \\
PROM--ANN Case~3
& $\mathbf q$
& $\mathbf u_{\mathrm{ref}}+\mathbf V\mathbf q+
   \overline{\mathbf V}\mathcal H(\mathbf q,\bm\mu,t)$
& $\mathbf V+\overline{\mathbf V}\dfrac{\partial\mathcal H}{\partial\mathbf q}$ \\
PROM--POD--AE
& $\mathbf z$
& $\mathbf u_{\mathrm{ref}}+\mathbf V_{\mathrm{tot}}\mathcal D(\mathbf z)$
& $\mathbf V_{\mathrm{tot}}\dfrac{\partial\mathcal D}{\partial\mathbf z}$ \\
\bottomrule
\end{tabular}
\end{table}
\noindent\emph{Common online principle.} For every row in
Table~\ref{tab:intrusive-trial-maps}, the online state is obtained by
minimizing the same time-discrete residual over the displayed trial map.  In a
PROM this residual is evaluated in full; in an HPROM its corresponding
projected normal equations are approximated by the model-specific ECM rule.
The non-intrusive POD--NN-ROM and POD--DL-ROM maps are excluded because they
perform no online residual minimization.
...
Given the campaign-consistent linear-solver targets $\mathbf q_N^{\mathrm{ref}}(t;\bm\mu)$, one minimizes a supervised loss (e.g., MSE)
\begin{equation}
...
of using projected HDM snapshots as labels.  Online POD--NN-ROM prediction is
a direct forward evaluation of $\mathcal G_q$ followed by reconstruction with
$\mathbf V_{\mathrm{tot}}$.
...
Section~\ref{sec:method-podae}: reconstruction of the corresponding
linear-solver $\mathbf q_N$ snapshots through a latent bottleneck. The POD-DL-ROM dynamics map then
...
$N_s=9\times501=4509$ snapshots.

\paragraph{Common basis configuration.}
The nine trajectories provide $4500$ candidate non-initial snapshot columns.
For the reported basis, we retain $2250$ centered snapshot columns and $2250$
metric samples through the same global parameter--time-stratified rule, with
uniform parameter weights and trapezoidal time weights.  In
Eq.~\eqref{eq:mlspg-metric}, we set $\mathbf P_s=\mathbf I$ and choose the
mass regularization by the trace-ratio rule
\begin{equation}
...
energy, so the mass term regularizes the metric scale without overriding the
LSPG-sensitivity contribution.

Stage~2 is performed at the same level as
...
same $n_{\mathrm{tot}}=151$ trial space.  Stage~3 then trains each learned
model from the reduced-coordinate trajectories
generated by the corresponding linear solver: PROM-generated trajectories in
the PROM campaign and linear-HPROM trajectories in the HPROM campaign.  Thus,
the learning labels are not HDM snapshots subsequently projected onto the
trial space.  This choice keeps each learned model aligned with the coordinate
dynamics of the reference solver used in its own campaign, rather than mixing
full-residual or sampled-residual effects with an additional projection-label
error.  A conventional purely non-intrusive workflow could instead use such
projected HDM labels; that alternative is not used in the present comparison.

\paragraph{Shared Case~2 and POD--NN master map.}
For the reported Case~2/POD--NN comparison, we train one full-coordinate
parameter--time map per campaign,
\begin{equation}
\mathcal G_q:\; (\bm\mu,t)\longmapsto
\widehat{\mathbf q}_N\in\mathbb R^{n_{\mathrm{tot}}}.
\label{eq:master-q-map}
\end{equation}
This is a comparison-control decision, not a restriction of the Case~2
formulation. For a Case~2 split with $n$ primary coordinates, the tail is the
selected secondary block of this map,
\begin{equation}
\mathcal M_n(\bm\mu,t)
...
\end{equation}
Case~2 inserts $\mathcal M_n$ into the residual solve for the first $n$
coordinates, whereas POD--NN-ROM uses the complete vector
$\mathcal G_q(\bm\mu,t)$ directly. Therefore $n=0$ is exactly the
POD--NN-ROM prediction from the same trained map, rather than a merely similar
regressor. This shared-map construction is used only to make the reported
Case~2 and POD--NN results directly comparable; a Case~2 tail could otherwise
...
\paragraph{External parameter validation.}
The master parameter--time map
\(\mathcal G_q:(\bm\mu,t)\mapsto\q_{151}\), shared by Case~2 and
POD--NN--ROM, is assessed with two complete coefficient trajectories held out
before regression training:
\begin{equation}
\bm\mu^{\mathrm{val},1}=(4.5625,0.02625),\qquad
\bm\mu^{\mathrm{val},2}=(5.1875,0.01875).
\label{eq:external-parameter-validation}
\end{equation}
They lie in the north-west and south-east cells of the baseline grid,
respectively, and each contains all $501$ time samples.  Their PROM targets
are generated by the $151$-coordinate linear PROM, while their HPROM targets
are generated by the corresponding $151$-coordinate linear HPROM.  Thus no
training sample occurs at either held-out parameter: the validation measures
generalization in \(\bm\mu\) over the common temporal grid, rather than a
random time-snapshot split at a parameter already seen in training.  These
locations are excluded from the $9$-, $9+8$-, $9+18$-, and $9+36$-trajectory regression
fits, from ECM construction, and from the four reported online evaluations.  In
particular, the historical label \(\bm\mu^{(v)}\) denotes a reported
verification location and is not one of the external parameter-validation
points.

\paragraph{Response-aware Case--2 diagnostic.}
The production master map is trained by regression of the complete
\(151\)-coordinate history \(\q_{151}\); Case~2 uses only its secondary
block.  We additionally test a solver-aware objective for the baseline
\(n=10\) Case--2 map.  For parameter trajectory \(j\), let
\(\widehat{\mathbf Q}_{s,\theta}^{(j)}\in\mathbb R^{\bar n\times N_t}\)
denote the ANN-predicted secondary-coordinate history, and let
\(\mathbf Q_{p,\mathrm{ref}}^{(j)}\) and
\(\mathbf Q_{s,\mathrm{ref}}^{(j)}\) be the primary and secondary blocks of
the linear-PROM teacher.  We denote by
\(\mathcal S_{\mub_j}[\widehat{\mathbf Q}_{s,\theta}^{(j)}]\) the complete
primary-coordinate trajectory produced by the same full-residual Case--2
LSPG solve used online when supplied with that predicted tail.  The tested
objective is
\begin{equation}
\mathcal L_{\mathrm{resp}}(\theta)
=
\frac{1}{N_\mu}\sum_{j=1}^{N_\mu}
\left[
\frac{\left\lVert\widehat{\mathbf Q}_{s,\theta}^{(j)}-
\mathbf Q_{s,\mathrm{ref}}^{(j)}\right\rVert_F^2}
{\left\lVert\mathbf Q_{s,\mathrm{ref}}^{(j)}\right\rVert_F^2}
+
\alpha\,
\frac{\left\lVert\mathcal S_{\mub_j}
\left[\widehat{\mathbf Q}_{s,\theta}^{(j)}\right]-
\mathbf Q_{p,\mathrm{ref}}^{(j)}\right\rVert_F^2}
{\left\lVert\mathbf Q_{p,\mathrm{ref}}^{(j)}\right\rVert_F^2}
\right].
\label{eq:case2-response-aware-loss}
\end{equation}
Here \(N_\mu\) is the number of training parameter trajectories.  The first
term is the usual tail-regression loss; the second penalizes the error that
the predicted tail induces in the primary coordinates recovered by the
projection solve.  Because \(\mathcal S_{\mub_j}\) depends on \(\theta\)
only through the current tail prediction, evaluating it at every training
step requires actually re-solving the full \(N_t\)-step nonlinear Case--2
LSPG problem with that prediction; there is no shortcut around this forward
solve.  What is avoided is backpropagating through it: the response gradient is
instead obtained by discrete implicit differentiation of the converged LSPG
normal equations, using the per-step Gauss--Newton normal matrix and its
coupling to the previous time step, swept backward in time (a discrete
adjoint) to give the exact gradient of \(\mathcal L_{\mathrm{resp}}\) with
respect to the predicted tail history, which is then supplied directly to
the tail network's own backward pass.  Only the network's parameters are
backpropagated through; the Gauss--Newton iterations themselves are never
unrolled.  This keeps the repeated forward re-solves affordable at the small
training budget used below (Section~\ref{sec:case2-response-aware}): the
network is fine-tuned from the already-trained master map for a handful of
epochs, rather than trained from scratch under this objective.  The online
Case--2 formulation is unchanged.  This diagnostic therefore tests the
effect of the offline objective alone.

In addition to the baseline $3\times3$ training set, we use a nested PROM
coefficient-data ladder.  Its full level contains $36$ Latin-hypercube
parameter points: $18$ inside $\mathcal D$ and $18$ in the $25\%$ expanded box
\begin{equation}
  [3.9375,5.8125]\times[0.01125,0.03375].
\end{equation}
The full design has fixed seed 42.  Two deterministic nested subsets of this
same design are used at the PROM level: the early level retains four interior
and four margin points, while the intermediate level retains nine interior and
nine margin points.  The subsets follow a centre-seeded greedy maximin ordering
in the normalized expanded box; the margin prefix is seeded with one point in
each exterior direction.  Consequently, the four PROM training budgets are
$9$, $9+8=17$, $9+18=27$, and $9+36=45$ trajectories; no resampling is
introduced when passing from one level to the next.  No additional HDM
trajectory is computed: the corresponding data sets contain $4{,}509$,
$8{,}517$, $13{,}527$, and $22{,}545$ coefficient snapshots, respectively.
The full $9+36$ design is also used in the HPROM enrichment campaign, with
linear-HPROM coefficient targets; the $9+8$ and $9+18$ comparisons below are
reported only at the PROM level.  The four reported test points are excluded
from every training design; the two external parameter-validation trajectories in
Eq.~\eqref{eq:external-parameter-validation} remain excluded as well.

\begin{table}[H]
\centering
\caption{PROM coefficient data used for the nested enrichment comparison.  The
$9+8$ and $9+18$ designs are deterministic subsets of the full $9+36$ LHS
design.  The two held-out parameter-validation trajectories are reported for
completeness but are not training data.  The full $9+36$ design and the same
two held-out locations are used at the HPROM level with linear-HPROM
coefficient targets.}
...
\caption{Baseline training, held-out parameter-validation, and online
evaluation locations in parameter space.  The two diamonds mark
\(\bm\mu^{\mathrm{val},1}\) and \(\bm\mu^{\mathrm{val},2}\), whose complete
linear-PROM or linear-HPROM trajectories are reserved for regression
validation.  They are distinct from the four red-star online evaluation
locations.  The axes are shared with the three nested-enrichment designs below.}
\label{fig:enrichment-sampling-baseline}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.74\textwidth]{Figures/prom_only/prom_enrichment_sampling_early.png}
\caption{Early nested PROM enrichment.  Four interior and four margin points
are selected from the fixed full LHS design; they are not a separately sampled
design.  The four margin representatives cover the left, right, top, and bottom
expanded-box directions.  The two diamonds and four red stars remain excluded
from training.  The axes match the other parameter-space figures.}
\label{fig:enrichment-sampling-early}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.74\textwidth]{Figures/prom_only/prom_enrichment_sampling_intermediate.png}
\caption{Intermediate nested PROM enrichment.  Nine interior and nine margin
points are selected from the fixed full LHS design; they are not a separately
sampled design.  The two diamonds and four red stars remain excluded from
training.  The axes match Figs.~\ref{fig:enrichment-sampling-baseline},
\ref{fig:enrichment-sampling-early}, and \ref{fig:enrichment-sampling-expanded}.}
\label{fig:enrichment-sampling-intermediate}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.74\textwidth]{Figures/prom_only/prom_enrichment_sampling_points.png}
\caption{Expanded enriched training, held-out parameter-validation, and online
evaluation locations in parameter space.  The baseline $3\times3$ grid is
supplemented with the full $18$ interior and $18$ margin Latin-hypercube
training points, generated with the fixed linear teacher in the $25\%$ expanded
parameter box.  The two diamonds and four red stars are excluded from every
training design.  The axes match
Fig.~\ref{fig:enrichment-sampling-baseline}; the enrichment changes coefficient
coverage, not the HDM snapshot basis.}
...
map is the \(n=0\) endpoint and the $151$-coordinate linear PROM is the \(n=151\) endpoint.  The black curve is
the in-domain mean over \(\mub^{(v)},\mub^{(1)},\mub^{(2)}\); the extrapolatory point \(\mub^{(3)}\) remains a
separate curve.}
...
\subsection{Response-Aware Case--2 Training Diagnostic}
\label{sec:case2-response-aware}
Equation~\eqref{eq:case2-response-aware-loss} was evaluated as a controlled
baseline diagnostic.  The response-aware network was initialized from the
selected Case--2/POD--NN master map, trained for six complete trajectory passes
over the same nine PROM parameter trajectories, and selected using the same
two external parameter-validation trajectories.  The coefficient \(\alpha\)
was set automatically by matching the two normalized losses at
initialization, giving \(\alpha=1373.21\).  A centered directional
finite-difference audit of the implicit gradient agreed with its adjoint
evaluation to a relative discrepancy of \(1.9\times10^{-7}\).

\begin{table}[H]
\centering
\caption{Baseline Case--2 response-aware training diagnostic for \(n=10\).
Entries are online state errors
\(100\lVert\uvec_{\mathrm{HDM}}-\uvec_{\mathrm{ROM}}\rVert_F/
\lVert\uvec_{\mathrm{HDM}}\rVert_F\).  The mean averages
\(\mub^{(v)},\mub^{(1)},\mub^{(2)}\).  The response-aware model is
initialized from the selected standard master map.  Both evaluations use the
same basis, full-residual online solver, and external parameter-validation
set; they differ only in the offline learning objective.}
\label{tab:prom-case2-response-aware}
\input{tables/prom_only/prom_case2_response_aware_diagnostic.tex}
\end{table}

The response penalty changes the in-domain mean only from \(1.370\%\) to
\(1.357\%\), while the extrapolatory error changes from \(1.898\%\) to
\(1.921\%\).  Thus, although the gradient audit verifies the response-aware
implementation, the tested response-aware objective and training budget
provide no robust improvement over the selected standard master map.  The
production comparisons consequently retain the standard shared-map training
objective.

\section{Effect of PROM Data Enrichment}
\label{sec:prom-enrichment}
...
online PROM formulations are also unchanged.  Only the training coefficient data vary through the nested
trajectory budgets $9$, $9+8$, $9+18$, and $9+36$.

\begin{table}[H]
\centering
\caption{Training and validation coefficient errors across the nested PROM data-enrichment ladder.  For the master
parameter--time map shared by Case~2 and POD--NN--ROM, validation uses the two complete held-out parameter
trajectories in Eq.~\eqref{eq:external-parameter-validation}; the state-conditioned closures and the autoencoder
retain their documented snapshot-level validation splits.}
...
\(4.391\%\) with nine trajectories to \(1.625\%\) with the nested $9+8$ set, \(0.814\%\) with the nested $9+18$
set, and \(0.555\%\) with the full $9+36$ set.  This is evidence that the baseline difficulty is primarily a
coefficient-data coverage limitation rather than an intrinsic impossibility of learning the map in this benchmark.
The POD--DL map also improves strongly, while the already accurate autoencoder still benefits.

\begin{table}[H]
\centering
\caption{PROM state errors across the nested coefficient-data ladder
\(100\norm{\uvec_{\mathrm{HDM}}-\uvec_{\mathrm{ROM}}}_F/\norm{\uvec_{\mathrm{HDM}}}_F\).  Each in-domain mean averages
\(\mub^{(v)},\mub^{(1)},\mub^{(2)}\); \(\mub^{(3)}\) is the extrapolation point.  The linear PROM is identical in all four
blocks because its basis and full residual formulation are fixed.  The horizontal rule separates residual-evaluating
PROMs from the two direct maps.}
\label{tab:prom-enrichment-online}
{\footnotesize
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.05}
\input{tables/prom_only/prom_enrichment_online_state_comparison.tex}}
\end{table}

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{Figures/prom_only/prom_enrichment_state_error_comparison.png}
\caption{Effect of the four-level nested PROM coefficient-data ladder on online state errors.  Left: mean error over the
verification and two off-grid in-domain points.  Right: extrapolatory point.  The vertical rule separates the
residual-evaluating intrusive PROM models from the non-intrusive direct ROM maps.}
...
The $9+8$ level resolves the transition rather than merely interpolating the
endpoints.  For Case~2 with $n=10$, the in-domain mean falls from \(1.370\%\)
to \(0.694\%\), while the extrapolatory error falls from \(1.898\%\) to
\(1.505\%\).  The corresponding POD--NN--ROM values are \(0.735\%\) and
\(2.371\%\), so residual minimization already improves upon the direct use of
the same master map.  However, POD--DL--ROM is still more accurate at this
budget, at \(0.471\%\) in-domain and \(1.251\%\) at extrapolation.  Case~3 is
the strongest intrusive result at $9+8$, with \(0.447\%\) and \(0.993\%\),
respectively.  Thus the early subset gives a genuine but model-dependent
intrusive advantage, not a uniform ordering.  At $9+18$, Case~2 reaches
\(0.489\%\) in-domain and \(0.891\%\) at extrapolation; the full $9+36$ set
gives only a smaller further reduction to \(0.469\%\) and \(0.872\%\).  At
that level all learned models are close to the fixed linear PROM at the
reported points; the exterior LHS samples remove most of the remaining
extrapolation gap.

\begin{table}[H]
\centering
\caption{PROM coefficient errors across the nested coefficient-data ladder, against the corresponding fixed
\(n_{\mathrm{tot}}=151\) linear PROM coefficient trajectory.  Values are relative Frobenius percentages over all
coefficients and time steps; each in-domain mean averages \(\mub^{(v)},\mub^{(1)},\mub^{(2)}\).  The horizontal rule
separates residual-evaluating PROMs from the two direct maps.}
\label{tab:prom-enrichment-coeff}
{\footnotesize
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.05}
\input{tables/prom_only/prom_enrichment_online_coeff_comparison.tex}}
\end{table}

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{Figures/prom_only/prom_enrichment_coeff_error_comparison.png}
\caption{Effect of the four-level nested PROM coefficient-data ladder on online coefficient errors.  The coefficient metric is
stricter than the state metric because it measures the entire \(151\)-coordinate trajectory against the linear PROM
teacher.  The vertical rule separates intrusive PROM models from the non-intrusive direct ROM maps.}
...
coefficient error of \(3.557\%\) and \(7.444\%\) at the extrapolatory point.  The nested $9+8$ set lowers these to
\(1.473\%\) and \(5.348\%\), showing that limited coverage improves the map but does not yet resolve its exterior
tail.  The $9+18$ set lowers them to \(0.585\%\) and \(0.893\%\), and the full $9+36$ set lowers them further to
\(0.398\%\) and \(0.805\%\), respectively.  Case~2 with \(n=10\) follows the same trend because it uses the same
master map for the injected tail.
...
\caption{Case--2 per-coefficient relative error across the four-level nested PROM coefficient-data ladder for \(n=10\).  The
vertical line marks the separation between the ten residual-solved primary coordinates and the ANN-injected
secondary tail.  The linear PROM coefficient trajectory is the reference.}
...
is moderate.  The nested $9+8$ set already reduces those errors over the full coefficient range; the $9+18$ and
$9+36$ sets provide successive, smaller reductions.  Throughout the ladder, the LSPG solve continues to recover the leading ten
coordinates.  This explains why the Case--2 residual becomes much less feedback-sensitive as coefficient coverage
is added.
...
\resizebox{0.9\textwidth}{!}{\input{tables/hprom_only/hprom_ecm_setup.tex}}
\end{table}
...
point.  Case 1, Case 3, and PROM--POD--AE remain close to this reference in the three in-domain tests, with mean
errors of \(0.533\%\), \(0.494\%\), and \(0.542\%\), respectively.  Case 2 remains the more sensitive closure:
with \(n=10\), its mean error is \(1.510\%\) and its extrapolatory error is \(4.272\%\).  Increasing the solved
primary dimension to \(n=20\) reduces these values to \(1.262\%\) and \(3.004\%\), respectively, but does not
match the robustness of Case 1, Case 3, or PROM--POD--AE.
...
The overlay in Fig.~\ref{fig:hprom-solution-overlays} provides the trajectory-level counterpart to the aggregate
errors.  The $n=10$ Case--2 solution is visibly less accurate at the off-grid and extrapolatory points than the
linear HPROM and the other intrusive closures, although the corrected production rule no longer produces the severe
departure seen in the earlier campaign.  Solving twenty coordinates reduces the state error, consistent with more of
the leading coefficient subspace being corrected by the online residual solve; the remaining difference is governed
by the ANN-supplied secondary block.
...
The HPROM coefficient diagnostics mirror the PROM-level mechanism.  The high-index tail remains the difficult part
of the learned map, and Case 2 is the most exposed to this difficulty because its tail is injected directly into the
online residual.  Increasing from \(n=10\) to \(n=20\) reduces the Case--2 coefficient error from \(3.371\%\) to
\(2.801\%\) in-domain mean and from \(9.996\%\) to \(6.981\%\) at extrapolation, but it does not remove the tail
limitation.  Case 3 is the most accurate learned HPROM in coefficient space, with \(0.612\%\) in-domain mean error,
because it combines residual-solved primary coordinates with explicit \((\mu_1,\mu_2,t)\) information in the
closure.
...
The direct map has an in-domain mean error of $1.607\%$ and an extrapolatory error of $7.401\%$.  With the production $n=10$ split, these become $1.509\%/4.257\%$; they then decrease to $1.261\%/3.005\%$ at $n=20$, $1.198\%/3.096\%$ at $n=30$, and $1.089\%/2.822\%$ at $n=50$.  The slight extrapolatory increase from $n=20$ to $n=30$ confirms that the trend is not strictly monotone.  All intermediate values remain above the linear-HPROM reference ($0.448\%/0.868\%$), but the reported dimensions quantify the benefit of enlarging the residual-solved block while retaining a learned secondary tail.
...
\begin{table}[H]
\centering
\caption{HPROM online wall time and speed-up relative to the fixed HDM reference
\(T_{\mathrm{HDM}}=737.44\,\mathrm{s}\), the mean wall time of the nine HDM training trajectories.
Intrusive times are means over the four evaluation trajectories and exclude offline ECM construction.  The linear
HPROM is the same fixed basis and ECM rule in both campaigns; its baseline mean is therefore reproduced in the
enriched column.  The horizontal rule separates sampled-residual HPROMs from the two direct non-intrusive maps.
Direct-map times are means from ten repeated loaded-network forward predictions at each evaluation point, excluding
checkpoint loading, state reconstruction, HDM evaluation, and file I/O.}
\label{tab:hprom-online-timing}
\resizebox{0.88\textwidth}{!}{\input{tables/hprom_only/hprom_enrichment_online_timing_comparison.tex}}
\end{table}

The sampled-residual closures provide a substantial online reduction relative to the HDM.  For Case--2 with
\(n=10\), the mean time is \(6.03\,\mathrm{s}\) in the baseline campaign and \(5.99\,\mathrm{s}\) after
enrichment, corresponding to \(122.2\times\) and \(123.0\times\) speed-ups relative to the fixed HDM reference.
The direct POD--NN--ROM and POD--DL--ROM maps do not solve a residual minimization problem; their entries in
Table~\ref{tab:hprom-online-timing} provide computational context rather than an ECM comparison.
...
Coefficient-data enrichment largely eliminates the state-level Case--2 gap.  At \(n=10\), its in-domain mean state
error falls from \(1.510\%\) to \(0.457\%\), and the extrapolatory error falls from \(4.272\%\) to \(0.912\%\).  The
latter is close to the \(0.868\%\) linear-HPROM reference.  Case 1 and Case 3 likewise return to the fixed linear
HPROM level, with enriched in-domain means of \(0.448\%\) and \(0.445\%\), respectively.  PROM--POD--AE remains
accurate, at \(0.467\%\) in-domain mean and \(0.874\%\) at extrapolation.  The production paired comparison keeps
\(n=10\) as the Case--2 closure; the \(n=20\) run is retained as a primary-dimension diagnostic rather than treated
as a separate learned model family.
...
Coefficient space confirms that the improvement is not merely cancellation in the state norm.  The Case--2
\(n=10\) in-domain mean coefficient error decreases from \(3.371\%\) to \(0.348\%\), and its extrapolatory
coefficient error decreases from \(9.996\%\) to \(0.681\%\).  The linear HPROM reference is unchanged; the observed
...
the paired enrichment figures retain the production \(n=10\) closure for a one-to-one comparison with the PROM
study.
...
\begin{enumerate}
  ...
  limiting task.  This affects both the POD--NN--ROM and the Case 2 injected tail.  A response-aware Case--2
  penalty that directly penalizes the induced primary-coordinate error changes the in-domain mean only from
  $1.370\%$ to $1.357\%$ and increases the extrapolatory error from $1.898\%$ to $1.921\%$; it does not resolve
  the data-coverage limitation in the tested configuration.
  \item Solving more primary coordinates in Case 2 can help, but the matched-rule HPROM sweep is not strictly
  monotone: the extrapolatory error is $4.257\%$ at $n=10$, $3.005\%$ at $n=20$, $3.096\%$ at $n=30$, and $2.822\%$
  at $n=50$, without reaching the $0.868\%$ linear-HPROM reference.  With a fixed $n=10$ rule, imposed tail error
  changes the primary coordinates recovered by the residual solve; tail accuracy is therefore essential.
  \item PROM data enrichment changes the picture substantially.  Keeping the architecture and basis fixed, the
  nested $9+8$ PROM set establishes the transition regime: it improves Case--2 relative to the direct master map,
  but does not yet dominate every direct ROM.  The $9+18$ set removes most of the remaining baseline Case--2 and
  direct-map error, and the full $9+36$ set reduces validation and online coefficient errors further, bringing the
  learned state errors close to the linear PROM level.
  ...
  or adding HDM training snapshots.  The \(9+36\) campaign reduces Case--2 \(n=10\) from \(1.510\%\) to \(0.457\%\)
  in-domain and from \(4.272\%\) to \(0.912\%\) at extrapolation, placing it on the same practical accuracy scale as
  the linear HPROM.
...
\(n\in\{0,10,20,30,50\}\) are shown because the HPROM diagnostic was run
...
\section{Low-Rank Secondary-Tail Correction Diagnostic}
\label{app:case2-tail-correction}

We also tested a low-rank correction of the Case--2 secondary tail.  The
objective was to give the residual solve a small number of additional
directions that represent typical parameter--time-map errors, without solving
all \(141\) secondary coordinates.  This is distinct from increasing the
primary dimension: the extra unknowns correct patterns in the secondary block.

Let \(\mathcal M_s(\mub,t)\in\mathbb R^{\bar n}\) denote the secondary block
of the shared parameter--time master map.  We used three parameter-group
out-of-fold fits on the baseline \(3\times3\) grid.  Each fit was trained on
six parameter trajectories, its errors were evaluated on the three held-out
parameter trajectories, and the two external parameter trajectories were used
only for early stopping.  Stacking these out-of-fold tail errors gives the
matrix \(\mathbf E\).  Thus, the directions below are not obtained from
in-sample ANN residuals.

Because the LSPG-sensitive basis is not Euclidean orthonormal, the correction
space was constructed in the physical tail-state norm.  Let
\(\mathbf T^T\mathbf T=\V_s^T\V_s\), where \(\V_s\) denotes the secondary
basis block, and let \(\mathbf W_t\) contain the trapezoidal time weights.
From the direct SVD
\begin{equation}
  \mathbf T\mathbf E\mathbf W_t^{1/2}=\mathbf U\mathbf\Sigma\mathbf Z^T,
  \qquad
  \mathbf B_r=\mathbf T^{-1}\mathbf U_r,
  \qquad
  \mathbf B_r^T\V_s^T\V_s\mathbf B_r=\mathbf I_r,
  \label{eq:case2-tail-correction-basis}
\end{equation}
where \(\mathbf U_r\) contains the first \(r\) columns of \(\mathbf U\),
the corrected Case--2 manifold becomes
\begin{equation}
  \widetilde{\uvec}
  =\uvec_{\mathrm{ref}}+\V_p\qp+\V_s
  \left[\mathcal M_s(\mub,t)+\mathbf B_r\mathbf a\right],
  \qquad
  (\qp,\mathbf a)\in\mathbb R^{10}\times\mathbb R^r.
  \label{eq:case2-tail-correction-manifold}
\end{equation}
The full-residual LSPG solve treats both \(\qp\) and \(\mathbf a\) as online
unknowns.  Since the Case--2 map is independent of \(\qp\), its tangent is
\begin{equation}
  \left[\V_p\ \ \V_s\mathbf B_r\right].
  \label{eq:case2-tail-correction-tangent}
\end{equation}
For \(r=0\), this formulation recovers the standard Case--2 solve exactly;
we verified a zero difference in the saved verification-point coordinate
trajectory.

\begin{table}[H]
\centering
\caption{Low-rank secondary-tail correction diagnostic for baseline
Case--2.  Entries are online state errors
\(100\lVert\uvec_{\mathrm{HDM}}-\uvec_{\mathrm{ROM}}\rVert_F/
\lVert\uvec_{\mathrm{HDM}}\rVert_F\).  The mean averages
\(\mub^{(v)},\mub^{(1)},\mub^{(2)}\).  The \(r\) correction coordinates are
in addition to the ten standard primary coordinates.}
\label{tab:case2-tail-correction}
\input{tables/prom_only/prom_case2_tail_correction_oof.tex}
\end{table}

The rank-one correction gives a small reduction at the two off-grid points and
at extrapolation, but that reduction is not reinforced by the higher-rank
spaces.  In particular, the in-domain mean moves from \(1.370\%\) to
\(1.333\%\) at \(r=1\), then returns to \(1.366\%\) at \(r=10\); the
extrapolatory error rises to
\(1.962\%\) at \(r=10\), above the \(1.898\%\) standard Case--2 value.
Thus the out-of-fold error basis does not supply a rank-robust low-dimensional
correction for this data set.  We therefore do not use \(\mathbf B_r\) in the
production comparisons.  The diagnostic is retained to distinguish this
negative result from a failure of the Case--2 formulation itself.

\paragraph{Observability-weighted variant.}
The construction above selects directions purely by out-of-fold error
magnitude, in the physical tail-state norm.  It does not ask whether the
residual-solved primary block can actually respond to a given secondary
direction.  We therefore also tested a second basis that rescales every
column of \(\mathbf E\) by the low/high LSPG transfer gain of
Section~\ref{sec:metric-aware-basis} before the same weighted SVD.  For a
sampled state with low/high transfer operator \(\mathbf T_{LH}\)
\eqref{eq:low-high-transfer} and an out-of-fold error column \(\mathbf e\),
define the physical-mass-norm gain
\begin{equation}
  g_M(\mathbf e)
  =\frac{\norm{\V_p\mathbf T_{LH}\mathbf e}_M}{\norm{\V_s\mathbf e}_M},
  \label{eq:case2-tail-correction-gain}
\end{equation}
i.e., how much of a secondary perturbation in that direction the primary
block's own Gauss--Newton update is expected to absorb.  Each column of
\(\mathbf E\) is rescaled by its own \(g_M(\mathbf e)\), evaluated along the
linear-PROM reference trajectory, before the SVD in
\eqref{eq:case2-tail-correction-basis}; the resulting \(\mathbf B_r\) is
state-orthonormalized exactly as before.  We separately verified that
\(g_M\) evaluated at the linear-reference states used to build the basis
agrees with \(g_M\) evaluated along the actual, already-drifted online
Case--2 rollout to four significant figures at every evaluation point,
because the residual Jacobian at this \(\Delta t\) is dominated by its
near-identity term; the cheaper linear-reference evaluation used offline is
therefore not itself a source of error in this comparison.

Random directions in the secondary tail-state space have a mean gain of
\(1.2\times10^{-3}\), roughly ten times below the largest gain the basis
admits at any point (\(1.6\)--\(2.3\times10^{-2}\)); the unweighted
out-of-fold error directions used above are statistically indistinguishable
from random in this sense (mean gain \(1.2\times10^{-3}\)).  Weighting by
\(g_M\) shifts the selected directions toward the observable end (mean gain
\(5.1\times10^{-3}\), an order of magnitude more concentrated), which is the
intended effect of \eqref{eq:case2-tail-correction-gain}.

\begin{table}[H]
\centering
\caption{Observability-weighted low-rank secondary-tail correction
diagnostic for baseline Case--2, same metric and columns as
Table~\ref{tab:case2-tail-correction}.}
\label{tab:case2-tail-correction-obsweighted}
\input{tables/prom_only/prom_case2_tail_correction_obsweighted.tex}
\end{table}

Reweighting by observability changes the qualitative failure mode but does
not remove it.  The in-domain mean is no longer degraded by \(r=10\)
(\(1.336\%\), against \(1.370\%\) standard Case--2 and \(1.366\%\) for the
unweighted basis at the same rank), and the extrapolatory error at \(r=10\)
now falls below the standard Case--2 value (\(1.859\%\) against
\(1.898\%\)), reversing the unweighted basis's worst result.  However, the
sweep is still non-monotone -- \(r=1\) is worse than the unweighted basis's
\(r=1\) on every column -- and the single best result in either table is not
the observability-weighted \(r=10\) but the unweighted basis's \(r=1\)
(\(1.333\%\) in-domain, \(1.835\%\) extrapolatory, from
Table~\ref{tab:case2-tail-correction}), obtained at a fifth of the online
dimension.  Observability weighting therefore relocates and mildly improves
the best achievable operating point; it does not make the correction
reliable enough to select a rank a priori, and we do not use either
\(\mathbf B_r\) construction in the production comparisons.

The residual non-monotonicity is consistent with a rank deficit rather than
a basis-selection error: an oracle rank-\(r\) approximation of the true
out-of-fold error energy itself, with no observability weighting or online
feedback at all, captures only \(10.6\%\), \(23.6\%\), and \(48.5\%\) of that
energy at \(r=1\), \(3\), and \(10\), respectively, because its singular
values decay slowly (a factor of about \(2\) from the first to the tenth).
No choice of \(r\le10\) directions, however selected, can remove more than
about half of the injected tail error's energy in this data set; the
remaining, higher-rank error is exactly what production enrichment
(Section~\ref{sec:prom-enrichment}) removes instead, by improving the map
\(\mathcal M\) itself rather than compensating for it online.

\section{Tangent-Preserving Oracle Diagnostics for State-Dependent Closures}
\label{app:state-dependent-oracles}

The Case--2 perturbation experiment is possible because its closure supplies a
parameter--time table independently of the online primary-coordinate iterate.
An analogous direct replacement of the secondary coordinates is not suitable
for Case~1 or Case~3: it would remove the state dependence of the closure and,
therefore, change the tangent used by the LSPG solve.  This appendix instead
uses an oracle perturbation that changes the closure output along the linear
PROM reference path while leaving its derivative with respect to the primary
coordinates unchanged.

Let \(\mathbf q_{p,\mathrm{lin}}^m\) and \(\mathbf q_{s,\mathrm{lin}}^m\) denote the
primary and secondary blocks of the full linear-PROM trajectory at a fixed
parameter \(\mub\).  For \(c\in\{1,3\}\), write \(\mathcal F_c\) for the
native learned closure, with the unused \((\mub,t)\) arguments suppressed for
Case~1.  We form the time-indexed discrepancy
\begin{equation}
\mathbf d_c^m
=
\mathcal F_c\!\left(\mathbf q_{p,\mathrm{lin}}^m,\mub,t^m\right)
-\mathbf q_{s,\mathrm{lin}}^m,
\qquad
\eta_c
=
\frac{\left\|\mathbf d_c\right\|_F}
     {\left\|\mathbf q_{s,\mathrm{lin}}\right\|_F}.
\label{eq:case13-oracle-discrepancy}
\end{equation}
For a prescribed reference-path tail level \(\eta\), the diagnostic closure is
\begin{equation}
\mathcal F_c^{[\eta]}\!\left(\qp;\mub,t^m\right)
=
\mathcal F_c\!\left(\qp;\mub,t^m\right)
+
\left(\frac{\eta}{\eta_c}-1\right)\mathbf d_c^m.
\label{eq:case13-oracle-closure}
\end{equation}
The correction is independent of \(\qp\), and hence
\begin{equation}
\frac{\partial \mathcal F_c^{[\eta]}}{\partial\qp}
=
\frac{\partial \mathcal F_c}{\partial\qp}.
\label{eq:case13-oracle-tangent}
\end{equation}
Consequently, this experiment preserves exactly the state-dependent tangent
of each native closure.  At \(\eta=0\),
\(\mathcal F_c^{[0]}(\mathbf q_{p,\mathrm{lin}}^m;\mub,t^m)=\mathbf q_{s,\mathrm{lin}}^m\);
at \(\eta=\eta_c\), the correction vanishes and the native ANN is recovered
for every online iterate.  The time-indexed additive term in the Case--1
diagnostic is only an oracle device: it is not part of the Case--1 production
architecture.

The abscissa in Figures~\ref{fig:app-case1-tangent-oracle} and
\ref{fig:app-case3-tangent-oracle} is the controlled discrepancy \(\eta\)
on the \emph{linear-reference path}.  Because the primary coordinates change
during the subsequent nonlinear solve, the tail mismatch actually observed
along the new online trajectory need not equal \(\eta\).  This distinction is
essential for state-dependent closures: it measures the feedback caused by
the LSPG solve rather than only the static closure error.

At zero imposed discrepancy, both diagnostics recover the corresponding
linear-PROM state errors to the displayed precision and leave the
primary-coordinate mismatch below \(2.3\times10^{-3}\%\).  Thus, the oracle
construction itself does not introduce a basis or tangent penalty.
At a \(50\%\) reference-path tail discrepancy, Case~1 produces state errors
between \(4.78\%\) and \(5.78\%\) and primary-coordinate errors between
\(4.85\%\) and \(7.65\%\).  Case~3 is more feedback-sensitive at the
verification and first off-grid points: the corresponding state errors are
\(10.69\%\) and \(7.17\%\), while the primary-coordinate errors are
\(14.06\%\) and \(9.35\%\), respectively.  In those two Case--3 runs the
tail error observed along the online trajectory reaches \(101.5\%\) and
\(66.3\%\), although the prescribed reference-path level is \(50\%\).
Therefore, a state-dependent closure is not automatically self-correcting:
its coupling to the LSPG iteration can either attenuate or amplify a
secondary-coordinate error.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{Figures/appendix/prom_case1_tangent_oracle_sensitivity.png}
\caption{Case--1 tangent-preserving oracle diagnostic.  The horizontal axis
is the imposed secondary-coordinate discrepancy measured on the linear-PROM
reference path.  The star marks the native Case--1 ANN tail error, for which
the correction in \eqref{eq:case13-oracle-closure} vanishes.  Left: state
error against the HDM.  Right: error in the ten solved primary coordinates
against the linear PROM.  The ordinate limits are shared with
Figure~\ref{fig:app-case3-tangent-oracle}.}
\label{fig:app-case1-tangent-oracle}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{Figures/appendix/prom_case3_tangent_oracle_sensitivity.png}
\caption{Case--3 tangent-preserving oracle diagnostic, using the same
reference-path definition, axes, and star convention as
Figure~\ref{fig:app-case1-tangent-oracle}.  The difference between the
prescribed abscissa and the tail error observed along the online trajectory
quantifies state-dependent feedback; this is particularly visible at the
verification and first off-grid parameters.}
\label{fig:app-case3-tangent-oracle}
\end{figure}

\section{Burgers Basis-Selection Diagnostics}
\label{app:basis-selection-diagnostics}

The $M$-regularized LSPG-sensitive basis was initially selected through two
diagnostics.
...
This is a local, linearized indication that the modified metric can reduce a
low--high feedback gain in the LSPG normal equations.  It is not, by itself,
a guarantee of a uniformly smaller nonlinear online response; that question
...
specific test: it can motivate a protective basis choice for nonlinear
intrusive models in which learned high coordinates or decoded manifold
corrections may contain approximation error.  This diagnostic is not used as
a standalone performance result; its evidence is complemented by the direct
PROM comparison below.
...
\paragraph{PROM-level controlled-tail comparison.}
Figure~\ref{fig:app-euclidean-lspg-tail} repeats the fixed-split Case--2
experiment with the Euclidean and the $M$-regularized LSPG-sensitive bases.
For each basis, a new full-residual linear PROM dataset and a new master
parameter--time map are trained using the same nine parameter trajectories
and the same two held-out parameter-validation trajectories.  The secondary
block is then perturbed along that basis-specific master-map error direction,
but rescaled to the common prescribed relative levels
$0,1,3,5,10,15,20,30,50\%$.  Hence the state-error panels are directly
comparable against the HDM, whereas the primary-coordinate panels report the
dimensionless feedback within each basis's own linear-PROM coordinate system.

The result is deliberately more qualified than a uniform metric-win claim.
At $50\%$ imposed tail error, the Euclidean basis gives smaller state and
primary-coordinate errors at the verification point and at $\bm\mu^{(2)}$,
the two bases are nearly indistinguishable at $\bm\mu^{(1)}$, and the
LSPG-sensitive basis is smaller at extrapolation.  The actual master-map tail
errors are also comparable but not identical, as indicated by the stars.
Thus the present four-trajectory nonlinear diagnostic does not establish a
uniform reduction of tail-to-primary error propagation.  Rather, it shows
that the linearized transfer criterion is a useful basis-selection diagnostic
whose online effect remains trajectory- and perturbation-direction-dependent.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{Figures/appendix/euclidean_vs_lspg_case2_n10_tail_sensitivity.png}
\caption{PROM-only controlled secondary-tail comparison for Case--2 with
$n=10$.  Rows are the verification point, two off-grid points, and the
extrapolatory point.  Blue circles denote the $M$-regularized LSPG-sensitive
POD basis and orange squares the Euclidean POD basis.  Both curves use the
same prescribed relative tail-error levels through $50\%$; stars identify the
actual tail-error level of the corresponding master parameter--time map.
Left: state error against the HDM.  Right: error in the ten solved primary
coordinates relative to that basis's own full linear PROM.}
\label{fig:app-euclidean-lspg-tail}
\end{figure}
