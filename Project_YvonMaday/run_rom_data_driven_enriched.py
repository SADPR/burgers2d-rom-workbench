...
def _load_rom_data_driven_model(model_path, device):
    if not os.path.exists(model_path):
        raise FileNotFoundError(
            ...
            "Run stage3_perform_training_rom_data_driven_enriched.py first."
        )
...
def main(
    ...
    ensure_layout_dirs()
    ensure_enrichment_dirs()
    os.makedirs(ENRICHMENT_RUNS_DATA_DRIVEN_DIR, exist_ok=True)
    ...
    run_tag = f"rom_data_driven_enriched_{tag}_ntot{total_modes}"
    out_dir = os.path.join(ENRICHMENT_RUNS_DATA_DRIVEN_DIR, run_tag)
    ...
    summary_path = os.path.join(out_dir, "rom_data_driven_enriched_summary.txt")
    write_kv_txt(
        summary_path,
        [
            ("mu_test", mu_test),
...
def _build_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional explicit checkpoint path. Default: latest enriched rom_data_driven model.",
    )
    return parser
