...
"""
Run enriched Case 2 ANN closure with enriched residual testing and selectable backend.

Case 2 mapping:
    qN_s = N(mu1, mu2, t)

Variants:
- solve_backend='prom': full PROM solve
...
from burgers.pod_ann_manifold import (
    compute_ECSW_training_matrix_2D_pod_ann_case2_petrov_galerkin,
    inviscid_burgers_implicit2D_LSPG_pod_ann_2D_case2_petrov_galerkin,
    inviscid_burgers_implicit2D_LSPG_pod_ann_2D_case2_petrov_galerkin_ecsw,
)
...
def _load_or_build_case2_pg_ecsw_weights(
    total_modes,
    n_primary,
    v,
    vbar,
    ...
    weights_path = os.path.join(
        weights_dir,
        f"ecsw_weights_ann_case2_pg_n{n_primary}_ntot{total_modes}{suffix}.npy",
    )
    ...
    for mu in mu_samples:
        ...
        ci = compute_ECSW_training_matrix_2D_pod_ann_case2_petrov_galerkin(
            snaps=snaps_now,
...
def _resolve_total_modes_from_checkpoint_or_dataset(ckpt):
    ...
    _, ntot, _, _, _ = resolve_enrichment_dataset(
        requested_ntot=None,
...
def _load_basis_and_reference(total_modes, n_primary):
    ...
    return v, vbar, u_ref, basis_path, uref_path


def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Run enriched Case 2 ANN with enriched residual testing and PROM/HPROM backend."
    )
    parser.add_argument("--backend", choices=("prom", "hprom"), default="prom")
    ...
    parser.add_argument(
        "--model-name",
        type=str,
        default="case2_model_enriched.pt",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional override for enriched checkpoint path (takes precedence over --model-name).",
    )
    parser.add_argument("--ecsw-tag", type=str, default="enriched")
    ...
    ecsw_tag = str(args.ecsw_tag)

    device = str(args.device).strip().lower()
    if device == "cuda" and not torch.cuda.is_available():
        print("[Case2-PG-Enriched] CUDA requested but not available. Falling back to CPU.")
        device = "cpu"

    set_latex_plot_style()
    ensure_layout_dirs()
    ensure_enrichment_dirs()
    runs_case_dir = ENRICHMENT_RUNS_CASE2_DIR
    ...
    if solve_backend == "hprom" and not use_ecsw:
        print("[Case2-PG-Enriched] solve_backend='hprom' with use_ecsw=False -> falling back to PROM solve.")
        effective_backend = "prom"
    if solve_backend == "prom" and use_ecsw:
        print("[Case2-PG-Enriched] use_ecsw=True ignored because solve_backend='prom'.")
    ...
    v, vbar, u_ref, basis_path, uref_path = _load_basis_and_reference(total_modes, n_p)

    w0 = np.asarray(W0, dtype=np.float64).reshape(-1).copy()
    if w0.size != v.shape[0]:
        raise ValueError(
            f"W0 size mismatch: got {w0.size}, expected {v.shape[0]} from basis."
        )

    snap_folder = os.path.join(PROJECT_ROOT, "Results", "param_snaps")
    os.makedirs(snap_folder, exist_ok=True)

    print(f"[Case2-PG-Enriched] device = {device}")
    print(f"[Case2-PG-Enriched] checkpoint = {model_path}")
    print(f"[Case2-PG-Enriched] basis = {basis_path}")
    print(f"[Case2-PG-Enriched] u_ref = {uref_path if os.path.exists(uref_path) else 'zeros'}")
    print(f"[Case2-PG-Enriched] solve_backend(requested) = {solve_backend}")
    print(f"[Case2-PG-Enriched] solve_backend(effective) = {effective_backend}")
    print(f"[Case2-PG-Enriched] use_ecsw = {use_ecsw}")
    ...
    if effective_backend == "hprom":
        ...
        t_ecsw0 = time.time()
        weights, weights_path, weights_source, ecsw_residual, n_ecsw_elements = _load_or_build_case2_pg_ecsw_weights(
        ...
        t_solve0 = time.time()
        red_coords, rom_times = inviscid_burgers_implicit2D_LSPG_pod_ann_2D_case2_petrov_galerkin_ecsw(
            grid_x=GRID_X,
            grid_y=GRID_Y,
            weights=weights,
            w0=w0,
        ...
        print(f"[Case2-PG-Enriched] ECSW weights: {weights_path} ({weights_source})")
        print(f"[Case2-PG-Enriched] ECSW training trajectories used = {ecsw_num_training_mu}")
        print(f"[Case2-PG-Enriched] N_e = {n_ecsw_elements}")
        print(f"[Case2-PG-Enriched] ECSW residual = {ecsw_residual}")

    else:
        t_solve0 = time.time()
        rom_snaps, rom_times = inviscid_burgers_implicit2D_LSPG_pod_ann_2D_case2_petrov_galerkin(
            grid_x=GRID_X,
            grid_y=GRID_Y,
            w0=w0,
            dt=DT,
    ...
    run_tag = f"case2_petrov_galerkin_{backend_tag}_ann_enriched_{tag}_n{n_p}_ntot{total_modes}"

    out_npy = os.path.join(runs_case_dir, f"{run_tag}_snaps.npy")
    ...
    plot_snaps(
        ...
        rom_snaps,
        plot_steps,
        label="HPROM-ANN Case 2 (PG test, enriched)" if effective_backend == "hprom" else "PROM-ANN Case 2 (PG test, enriched)",
        fig_ax=(fig, ax1, ax2),
        color="blue",
        linewidth=1.8,
        linestyle="solid",
    ...
    out_png = os.path.join(runs_case_dir, f"{run_tag}_hdm_vs_rom.png")
    ...
    write_kv_txt(
        summary_txt,
        [
            ("solver_variant", "case2_petrov_galerkin_enriched_residual_testing"),
            ("mu_test", mu_test),
            ("device", device),
    ...
    print(f"[Case2-PG-Enriched] ecsw_setup_elapsed = {ecsw_setup_elapsed:.3e} s")
    print(f"[Case2-PG-Enriched] online_solve_elapsed = {online_solve_elapsed:.3e} s")
    print(f"[Case2-PG-Enriched] elapsed = {elapsed:.3e} s")
    print(f"[Case2-PG-Enriched] its={num_its} | jac={jac_time:.3e} | res={res_time:.3e} | ls={ls_time:.3e}")
    print(f"[Case2-PG-Enriched] relative error vs HDM: {rel_err:.2f}%")
    print(f"[Case2-PG-Enriched] saved snaps: {out_npy}")
    print(f"[Case2-PG-Enriched] saved plot:  {out_png}")
    print(f"[Case2-PG-Enriched] summary:     {summary_txt}")
