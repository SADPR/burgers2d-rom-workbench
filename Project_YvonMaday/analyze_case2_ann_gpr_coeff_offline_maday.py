...
def _style_for_family_ns(family: str, n_s: int):
    # Explicit high-contrast style set (requested): red, blue, green, black, magenta, orange.
    key = (str(family).upper(), int(n_s))
    mapping = {
        ...
        ("ANN", 151): ("magenta", dict(linestyle="-", linewidth=2.8, alpha=0.78, marker="D", markersize=2.5, zorder=4)),
...
def main(argv=None) -> None:
    ...
    for (mu1, mu2) in points:
        ...
        for ax in axes:
            ...
            ax.axvline(10.5, linestyle="--", linewidth=0.9, color="gray", alpha=0.7)
            ax.axvline(20.5, linestyle="--", linewidth=0.9, color="gray", alpha=0.7)
