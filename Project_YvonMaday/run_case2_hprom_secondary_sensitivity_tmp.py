#!/usr/bin/env python3
"""Temporary HPROM Case-2 secondary-oracle sensitivity diagnostic.

For a fixed primary dimension n=10, prescribe the secondary coefficients
q_{n+1:151}(t) from the linear HPROM and contaminate them in the direction of
the actual master-ANN secondary error.  The HPROM solve then recomputes only
the first n coordinates with a fixed ECSW rule.  This isolates how
secondary-coordinate errors propagate into the recovered primary coordinates
and the state error, without changing the hyper-reduction rule.
...
class TimeTableSecondary(nn.Module):
    ...
    def forward(self, x_raw: torch.Tensor) -> torch.Tensor:
        ...
        if x.ndim == 1:
            ...
            return self.qbar_table[:, idx].to(device=x.device, dtype=x.dtype)
        if x.ndim == 2:
            idx = self._time_index(x[:, 2])
            return self.qbar_table.index_select(1, idx).T.to(device=x.device, dtype=x.dtype)
...
def solve_one_level(
    *,
    ...
    t0 = time.time()
    red_coords, stats = inviscid_burgers_implicit2D_LSPG_pod_ann_2D_case2_ecsw(
        grid_x=grid_x,
        grid_y=grid_y,
        weights=weights,
        w0=w0,
    ...
    n_t = min(red_coords.shape[1], qbar.shape[1])
    snaps = u_ref[:, None] + v @ red_coords[:, :n_t] + vbar @ qbar[:, :n_t]
    return snaps, red_coords[:, :n_t], stats, time.time() - t0
...
def run_point(
    *,
    ...
    linear_dir = hprom_root / "Runs" / "Linear" / f"linear_hprom_{tag}_ntot{n_tot}"
    ann_dir = hprom_root / "Runs" / "DataDriven_Best" / f"rom_data_driven_{tag}_ntot{n_tot}"
    ...
    weights = load_required_array(ecsw_weights_path, ndim=1, name="ECSW weights")
    ...
    if weights.size != (GRID_X.size - 1) * (GRID_Y.size - 1):
        raise ValueError(
            "ECSW weight count does not match the configured mesh: "
            f"{weights.size} vs {(GRID_X.size - 1) * (GRID_Y.size - 1)}"
        )
    if W0.size != u_ref.size:
        raise ValueError(f"Configured W0/u_ref size mismatch: {W0.size} vs {u_ref.size}")

    (qn_linear, qn_ann), n_t = align_time(qn_linear, qn_ann)
    ...
    hdm_snaps = hdm_snaps[:, : num_steps_eff + 1]
    lin_snaps = u_ref[:, None] + basis @ qn_linear
    ...
    for pct in point_levels:
        ...
        row = {
            ...
            "state_error_percent_vs_linear_hprom": state_vs_linear,
            "primary_q_error_percent_vs_linear_hprom": qp_err,
            "total_q_error_percent_vs_linear_hprom": qtot_err,
        ...
        with summary_path.open("w", encoding="utf-8") as f:
            ...
            f.write("backend: hprom\n")
            f.write(f"ecsw_weights_path: {ecsw_weights_path}\n")
            f.write(f"n_ecsw_elements: {int(np.count_nonzero(weights))}\n")
...
def write_csv(path: Path, rows: list[dict[str, object]]) -> None:
    ...
    fields = [
        ...
        "state_error_percent_vs_linear_hprom",
        "primary_q_error_percent_vs_linear_hprom",
        "total_q_error_percent_vs_linear_hprom",
        "n_time_used",
...
def plot_summary(rows: list[dict[str, object]], output_root: Path) -> tuple[Path, Path]:
    ...
    def _plot(ykey: str, ylabel: str, out_name: str) -> Path:
        ...
        ax.set_xlabel(r"imposed secondary error $\|e_s\|_F/\|q_s^{HPROM}\|_F$ (%)")
    ...
    primary_fig = _plot(
        "primary_q_error_percent_vs_linear_hprom",
        r"primary coefficient error against linear HPROM (%)",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Temporary HPROM Case-2 sensitivity to controlled secondary-coordinate errors."
    )
    ...
    parser.add_argument("--hprom-root", type=Path, default=THIS_DIR / "Results_Paper" / "mlspg_hprom_main")
    ...
    parser.add_argument("--output-root", type=Path, default=THIS_DIR / "Results_Paper" / "tmp_case2_hprom_secondary_sensitivity")
    ...
    print("[case2-hprom-secondary-sensitivity] backend: hprom")
    print("[case2-hprom-secondary-sensitivity] ECSW weights:", args.ecsw_weights_path)
    if args.plan_only:
        return
    ...
    for point in points:
        rows = run_point(
            point=point,
            levels=levels,
            hprom_root=args.hprom_root.expanduser().resolve(),
            basis_path=args.basis_path.expanduser().resolve(),
            u_ref_path=args.u_ref_path.expanduser().resolve(),
            ecsw_weights_path=args.ecsw_weights_path.expanduser().resolve(),
