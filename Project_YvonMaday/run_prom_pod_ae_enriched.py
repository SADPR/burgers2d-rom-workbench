...
"""
Run intrusive PROM-POD-AE ROM (enriched model) with selectable backend.
...
def _reconstruct_full_snaps_from_latent(latent_coords, pod_ae_model, basis_q, u_ref, device):
    ...
    snaps = np.zeros((int(u_ref.size), latent_coords.shape[1]), dtype=np.float64)
    for k in range(latent_coords.shape[1]):
        snaps[:, k] = _decode_snapshot_from_latent(latent_coords[:, k], pod_ae_model, basis_q, u_ref, device)
    return snaps
...
def _load_or_build_pod_ae_ecsw_weights(
    ...
    os.makedirs(ENRICHMENT_RUNS_ECSW_DIR, exist_ok=True)
    weights_path = os.path.join(
        ENRICHMENT_RUNS_ECSW_DIR,
        f"ecsw_weights_pod_ae_ntot{q_dim}_enriched.npy",
    )
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Run enriched PROM-POD-AE intrusive ROM with PROM or HPROM backend.")
    ...
    if device == "cuda" and not torch.cuda.is_available():
        print("[POD-AE-Enriched] CUDA requested but not available. Falling back to CPU.")
        device = "cpu"

    set_latex_plot_style()
    ensure_layout_dirs()
    ensure_enrichment_dirs()
    os.makedirs(ENRICHMENT_RUNS_POD_AE_DIR, exist_ok=True)

    effective_backend = solve_backend
    if solve_backend == "hprom" and not use_ecsw:
        print("[POD-AE-Enriched] solve_backend='hprom' with use_ecsw=False -> falling back to PROM solve.")
        effective_backend = "prom"
    if solve_backend == "prom" and use_ecsw:
        print("[POD-AE-Enriched] use_ecsw=True ignored because solve_backend='prom'.")
    ...
    if w0.size != basis_q.shape[0]:
        raise ValueError(f"W0 size mismatch: got {w0.size}, expected {basis_q.shape[0]} from basis.")

    snap_folder = os.path.join(PROJECT_ROOT, "Results", "param_snaps")
    os.makedirs(snap_folder, exist_ok=True)

    hdm_snaps = load_or_compute_snaps(
        mu=mu_test, grid_x=GRID_X, grid_y=GRID_Y, w0=w0, dt=DT, num_steps=NUM_STEPS, snap_folder=snap_folder
    )
    ...
    if effective_backend == "hprom":
        mu_train_candidates = get_snapshot_params(mu1_range=MU1_RANGE, mu2_range=MU2_RANGE, samples_per_mu=SAMPLES_PER_MU)
        ...
        rom_snaps = _reconstruct_full_snaps_from_latent(latent_coords, pod_ae_model, basis_q, u_ref, device)
        print(f"[POD-AE-Enriched] ECSW weights: {weights_path} ({weights_source})")
    else:
        t_solve0 = time.time()
    ...
    run_tag = f"podae_enriched_{backend_tag}_{tag}_ntot{q_dim}_nz{latent_dim}"

    out_snaps = os.path.join(ENRICHMENT_RUNS_POD_AE_DIR, f"{run_tag}_snaps.npy")
    out_latent = os.path.join(ENRICHMENT_RUNS_POD_AE_DIR, f"{run_tag}_latent.npy")
    ...
    fig, ax1, ax2 = plot_snaps(
        GRID_X, GRID_Y, hdm_snaps, plot_steps, label="HDM", color="black", linewidth=2.8, linestyle="solid"
    )
    plot_snaps(
        GRID_X,
        GRID_Y,
        rom_snaps,
        plot_steps,
        label="HPROM-POD-AE (Enriched)" if effective_backend == "hprom" else "PROM-POD-AE (Enriched)",
        fig_ax=(fig, ax1, ax2),
        color="blue",
        linewidth=1.8,
        linestyle="solid",
    ...
    ax2.legend()
    plt.tight_layout()
    out_plot = os.path.join(ENRICHMENT_RUNS_POD_AE_DIR, f"{run_tag}_hdm_vs_rom.png")
    plt.savefig(out_plot, dpi=200)
    plt.close(fig)

    summary_txt = os.path.join(ENRICHMENT_RUNS_POD_AE_DIR, f"{run_tag}_summary.txt")
    write_kv_txt(
        summary_txt,
        [
            ("mu_test", mu_test),
    ...
    print(f"[POD-AE-Enriched] relative error vs HDM: {rel_err:.2f}%")
    print(f"[POD-AE-Enriched] saved run folder base: {ENRICHMENT_RUNS_POD_AE_DIR}")
