...
"""
Runner for Case 2 PRNN-style closure ROM with selectable backend.
...
def _load_case2_model(model_path, device):
    ...
    return model, n_s, ckpt, hidden_dims, activation, dropout
...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Run Case 2 PRNN-style closure with PROM/HPROM backend."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="case2_prnn",
        help="Prefix for output filenames to avoid clashing with production runs.",
    )
    ...
    run_tag_prefix = str(args.run_tag_prefix).strip() or "case2_prnn"

    device = str(args.device).strip().lower()
    if device == "cuda" and not torch.cuda.is_available():
        print("[Case2-PRNN] CUDA requested but not available. Falling back to CPU.")
        device = "cpu"
    ...
    if solve_backend == "hprom" and not use_ecsw:
        print("[Case2-PRNN] solve_backend='hprom' with use_ecsw=False -> falling back to PROM solve.")
        effective_backend = "prom"
    if solve_backend == "prom" and use_ecsw:
        print("[Case2-PRNN] use_ecsw=True ignored because solve_backend='prom'.")
    ...
    base_model, n_s, ckpt, model_hidden_dims, model_activation, model_dropout = _load_case2_model(
    ...
    print(f"[Case2-PRNN] checkpoint = {model_path}")
    print(f"[Case2-PRNN] arch.hidden_dims = {model_hidden_dims}")
    print(f"[Case2-PRNN] arch.activation = {model_activation}")
    print(f"[Case2-PRNN] arch.dropout = {model_dropout}")
    print(f"[Case2-PRNN] train.omega_data = {ckpt.get('omega_data', 'N/A')}")
    print(f"[Case2-PRNN] train.omega_residual = {ckpt.get('omega_residual', 'N/A')}")
    print(f"[Case2-PRNN] train.physics_projection = {ckpt.get('physics_projection', 'N/A')}")
    print(f"[Case2-PRNN] basis = {basis_path}")
    print(f"[Case2-PRNN] u_ref = {uref_path if os.path.exists(uref_path) else 'zeros'}")
    print(f"[Case2-PRNN] solve_backend(requested) = {solve_backend}")
    print(f"[Case2-PRNN] solve_backend(effective) = {effective_backend}")
    print(f"[Case2-PRNN] use_ecsw = {use_ecsw}")
    ...
    if effective_backend == "hprom":
        ...
        print(f"[Case2-PRNN] ECSW weights: {weights_path} ({weights_source})")
        print(f"[Case2-PRNN] ECSW training trajectories used = {ecsw_num_training_mu}")
        print(f"[Case2-PRNN] N_e = {n_ecsw_elements}")
        print(f"[Case2-PRNN] ECSW residual = {ecsw_residual}")
    ...
    run_tag = f"{run_tag_prefix}_{backend_tag}_ann_prnn_{tag}_n{n_p}_ntot{total_modes}"
    ...
    plot_snaps(
        ...
        rom_snaps,
        plot_steps,
        label="HPROM-PRNN Case 2" if effective_backend == "hprom" else "PROM-PRNN Case 2",
        fig_ax=(fig, ax1, ax2),
        color="blue",
        linewidth=1.8,
        linestyle="solid",
    ...
    write_kv_txt(
        ...
        [
            ...
            ("model_omega_data", ckpt.get("omega_data", "N/A")),
            ("model_omega_residual", ckpt.get("omega_residual", "N/A")),
            ("model_physics_projection", ckpt.get("physics_projection", "N/A")),
            ("model_loss_form", ckpt.get("loss_form", "N/A")),
    ...
    print(f"[Case2-PRNN] ecsw_setup_elapsed = {ecsw_setup_elapsed:.3e} s")
    print(f"[Case2-PRNN] online_solve_elapsed = {online_solve_elapsed:.3e} s")
    print(f"[Case2-PRNN] elapsed = {elapsed:.3e} s")
    print(f"[Case2-PRNN] its={num_its} | jac={jac_time:.3e} | res={res_time:.3e} | ls={ls_time:.3e}")
    print(f"[Case2-PRNN] relative error vs HDM: {rel_err:.2f}%")
    print(f"[Case2-PRNN] saved snaps: {out_npy}")
    print(f"[Case2-PRNN] saved plot:  {out_png}")
    print(f"[Case2-PRNN] summary:     {summary_txt}")
