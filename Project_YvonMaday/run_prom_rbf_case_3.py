...
"""
Run Case 3 RBF closure ROM with selectable backend.
...
def _load_or_build_case3_ecsw_weights(
    ...
    weights_path = os.path.join(
        RUNS_ECSW_DIR,
        f"ecsw_weights_rbf_case3_n{n_primary}_ntot{total_modes}.npy",
    )
...
def _load_case3_model(model_path, device):
    ...
    if str(ckpt.get("format", "")).strip().lower() != "rbf_map":
        raise ValueError(
            f"{model_path} is not an RBF-map checkpoint. "
            "Train with stage3_perform_training_case_3_rbf.py first."
        )
    in_dim = int(ckpt["in_dim"])
    n_s = int(ckpt["out_dim"])
    n_p = int(in_dim - 3)
    if n_p < 1:
        raise ValueError(f"Invalid Case3 RBF in_dim={in_dim}; expected n_p+3 with n_p>=1.")
...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Run Case 3 RBF closure with PROM/HPROM backend."
    )
    ...
    rbf_model = ANNVectorWrapper(base_model).to(device)
    rbf_model.eval()
    ...
    run_tag = f"case3_{backend_tag}_rbf_{tag}_n{n_p}_ntot{total_modes}"
    ...
    plot_snaps(
        ...
        rom_snaps,
        plot_steps,
        label="HPROM-RBF Case 3" if effective_backend == "hprom" else "PROM-RBF Case 3",
        fig_ax=(fig, ax1, ax2),
        color="blue",
        linewidth=1.8,
        linestyle="solid",
    ...
    summary_txt = os.path.join(RUNS_CASE3_DIR, f"{run_tag}_summary.txt")
    write_kv_txt(
        summary_txt,
        [
            ("mu_test", mu_test),
