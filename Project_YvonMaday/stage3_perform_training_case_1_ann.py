...
def main(argv=None):
    ...
    if external_validation:
        print("[Case1] val_split = external validation dataset")
        print(f"[Case1] validation_dataset_dir = {args.validation_dataset_dir}")
    else:
        print("[Case1] val_split = row (train_test_split shuffle)")
        print(f"[Case1] val_frac = {val_frac}")
    print(
    ...
    if external_validation:
        ...
        Xva, Yva = load_prom_dataset_case1(
            validation_dataset_root,
            primary_modes=primary_modes,
        )
        if Xva.shape[1] != n_p or Yva.shape[1] != n_s:
            raise ValueError(
                "Validation data dimensions do not match training data: "
                f"Xva={Xva.shape}, Yva={Yva.shape}, expected (*,{n_p}) and (*,{n_s})."
            )
        Xtr, Ytr = X_raw, Y_raw
    else:
    ...
    print(f"[Case1] train_samples = {Xtr.shape[0]}")
    print(f"[Case1] val_samples = {Xva.shape[0]}")
