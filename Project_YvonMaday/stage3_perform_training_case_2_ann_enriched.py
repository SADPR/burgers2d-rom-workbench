...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Train enriched Case-2 ANN map from enrichment Stage-2 dataset."
    )
    ...
    model_name = str(args.model_name).strip() if args.model_name is not None else "case2_model_enriched.pt"
    if len(model_name) == 0:
        raise ValueError("--model-name cannot be empty.")
    if not model_name.endswith(".pt"):
        model_name = f"{model_name}.pt"
    model_path = os.path.join(stage3_models_dir, model_name)
    summary_path = os.path.join(stage3_out_dir, "case2_training_summary_enriched.txt")
    ...
    # Model
    # -----------------------------
    model = Case2Model(x_mean, x_std, y_mean, y_std).to(device)
