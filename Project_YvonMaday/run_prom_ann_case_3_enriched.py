...
def _load_or_build_case3_ecsw_weights(
    ...
    weights_path = os.path.join(
        weights_dir,
        f"ecsw_weights_ann_case3_n{n_primary}_ntot{total_modes}{suffix}.npy",
    )
...
def _load_case3_model(model_path, device):
    ...
    model = Case3Model(in_dim=in_dim, n_s=n_s).to(device)
...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Run enriched Case 3 ANN closure with PROM/HPROM backend."
    )
    ...
    parser.add_argument(
        "--model-name",
        type=str,
        default="case3_model_enriched.pt",
        help="Enriched checkpoint filename (used if --model-path is not set).",
    )
    parser.add_argument("--model-path", type=str, default=None, help="Optional override for enriched checkpoint path.")
    ...
    ensure_layout_dirs()
    ensure_enrichment_dirs()
    runs_case_dir = ENRICHMENT_RUNS_CASE3_DIR
    ...
    run_tag = f"case3_{backend_tag}_ann_enriched_{tag}_n{n_p}_ntot{total_modes}"
