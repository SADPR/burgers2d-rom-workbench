...
"""
Run Case 3 ANN closure ROM with selectable backend.
...
from burgers.pod_ann_manifold import (
    compute_ECSW_training_matrix_2D_pod_ann_case3,
    inviscid_burgers_implicit2D_LSPG_pod_ann_2D_case3,
    inviscid_burgers_implicit2D_LSPG_pod_ann_2D_case3_ecsw,
)
...
class LegacyCase3Model(nn.Module):
    """Input: [qN_p, mu1, mu2, t], output: qN_s."""

    def __init__(self, in_dim, n_s):
        super().__init__()
        self.scaler = Scaler(np.zeros((1, in_dim)), np.ones((1, in_dim)))
        self.core = LegacyCoreMLP(in_dim, n_s)
...
class ConfigurableCase3Model(nn.Module):
    """Input: [qN_p, mu1, mu2, t], output: qN_s."""

    def __init__(self, in_dim, n_s, hidden_dims, activation="elu", dropout=0.0):
        super().__init__()
        self.scaler = Scaler(np.zeros((1, in_dim)), np.ones((1, in_dim)))
...
def _reconstruct_case3_full_snaps(red_coords, basis, basis2, u_ref, ann_model, mu, dt, device="cpu"):
    ...
    for k in range(red_coords.shape[1]):
        yk = red_coords[:, k]
        t = float(k * dt)
        x = np.concatenate([yk, [mu1, mu2, t]]).astype(np.float32)
        with torch.no_grad():
...
def _build_case3_full_coordinates(red_coords, ann_model, mu, dt, device="cpu"):
    """Return [q_primary; H(q_primary, mu, t)] for every time step."""
    q_primary = np.asarray(red_coords, dtype=np.float64)
    if q_primary.ndim != 2:
        raise ValueError(f"red_coords must be 2D, got shape {q_primary.shape}")

    n_times = q_primary.shape[1]
    time_values = float(dt) * np.arange(n_times, dtype=np.float64)
    mu_block = np.column_stack(
        (
            np.full(n_times, float(mu[0]), dtype=np.float64),
            np.full(n_times, float(mu[1]), dtype=np.float64),
            time_values,
        )
    )
    model_inputs = np.hstack((q_primary.T, mu_block))
    with torch.no_grad():
...
def _load_or_build_case3_ecsw_weights(
    total_modes,
    n_primary,
    v,
    vbar,
    ...
    weights_path = os.path.join(
        weights_root,
        f"ecsw_weights_ann_case3{label}_n{n_primary}_ntot{total_modes}.npy",
    )
    ...
    for imu, mu in enumerate(mu_samples):
        ...
        ci = compute_ECSW_training_matrix_2D_pod_ann_case3(
            snaps=snaps_now,
...
def _load_case3_model(model_path, device):
    ...
    if in_dim != (n_p + 3):
        raise ValueError(
            f"Case3 checkpoint in_dim={in_dim}, expected n_p+3={n_p + 3}."
        )

    if "hidden_dims" in ckpt:
        model = ConfigurableCase3Model(
            in_dim=in_dim,
            n_s=n_s,
        ...
        model = LegacyCase3Model(in_dim=in_dim, n_s=n_s).to(device)
    model.load_state_dict(ckpt["state_dict"], strict=True)
    model.eval()

    return model, in_dim, n_p, n_s, ckpt
...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional isolated directory for Case-3 ECSW weights.",
    )
    ...
    if solve_backend == "hprom" and not use_ecsw:
        print("[Case3] solve_backend='hprom' with use_ecsw=False -> falling back to PROM solve.")
    ...
    ntot_ckpt = ckpt.get("dataset_ntot", None)
    if ntot_ckpt is not None and int(ntot_ckpt) != total_modes:
        raise ValueError(
            f"Case3 checkpoint mismatch: dataset_ntot={int(ntot_ckpt)} but n_p+n_s={total_modes}."
        )
    ...
    print(f"[Case3] checkpoint = {model_path}")
    print(f"[Case3] basis = {basis_path}")
    print(f"[Case3] u_ref = {uref_path if os.path.exists(uref_path) else 'zeros'}")
    ...
    if effective_backend == "hprom":
        ...
        print(f"[Case3] ECSW weights: {weights_path} ({weights_source})")
        print(f"[Case3] ECSW training trajectories used = {ecsw_num_training_mu}")
        print(f"[Case3] N_e = {n_ecsw_elements}")
        print(f"[Case3] ECSW residual = {ecsw_residual}")
        print(f"[Case3] ecsw_setup_elapsed = {ecsw_setup_elapsed:.3e} s")
        if args.ecsw_only:
            print("[Case3] ECSW-only mode complete; online solve skipped.")
            return
        ...
        qN = _build_case3_full_coordinates(
            red_coords=red_coords,
            ann_model=ann_model,
            mu=mu_test,
            dt=DT,
    ...
    run_tag = f"case3_{backend_tag}_ann_{tag}_n{n_p}_ntot{total_modes}"
    ...
    plot_snaps(
        ...
        rom_snaps,
        plot_steps,
        label="HPROM-ANN Case 3" if effective_backend == "hprom" else "PROM-ANN Case 3",
        fig_ax=(fig, ax1, ax2),
        color="blue",
        linewidth=1.8,
        linestyle="solid",
    ...
    print(f"[Case3] saved qN:    {qn_output}")
    print(f"[Case3] saved snaps: {out_npy}")
    print(f"[Case3] saved plot:  {out_png}")
    print(f"[Case3] summary:     {summary_txt}")
