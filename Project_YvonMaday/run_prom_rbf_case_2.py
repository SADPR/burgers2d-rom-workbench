...
"""
Run Case 2 RBF closure ROM with selectable backend.
...
def _load_or_build_case2_ecsw_weights(
    ...
    weights_path = os.path.join(
        RUNS_ECSW_DIR,
        f"ecsw_weights_rbf_case2_n{n_primary}_ntot{total_modes}.npy",
    )
...
def _load_case2_model(model_path, device):
    ...
    if str(ckpt.get("format", "")).strip().lower() != "rbf_map":
        raise ValueError(
            ...
            "Train with stage3_perform_training_case_2_rbf.py first."
        )
    n_s = int(ckpt["out_dim"])
    in_dim = int(ckpt.get("in_dim", 3))
    if in_dim != 3:
        raise ValueError(f"Case2 RBF checkpoint in_dim={in_dim}, expected 3")
...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Run Case 2 RBF closure with PROM/HPROM backend."
    )
    ...
    run_tag = f"case2_{backend_tag}_rbf_{tag}_n{n_p}_ntot{total_modes}"
    ...
    plot_snaps(
        ...
        rom_snaps,
        plot_steps,
        label="HPROM-RBF Case 2" if effective_backend == "hprom" else "PROM-RBF Case 2",
        fig_ax=(fig, ax1, ax2),
        color="blue",
        linewidth=1.8,
        linestyle="solid",
