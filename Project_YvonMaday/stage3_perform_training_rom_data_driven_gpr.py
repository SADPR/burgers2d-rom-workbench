...
"""Train full data-driven GPR map: qN = G_gpr(mu1, mu2, t)."""
...
def _split_indices_by_mu(x_raw: np.ndarray, val_frac: float, rng: np.random.Generator):
...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Train full data-driven GPR map qN = G_gpr(mu1,mu2,t)."
    )
    ...
    parser.add_argument("--model-name", type=str, default="rom_data_driven_gpr_model.pt")
    parser.add_argument("--summary-name", type=str, default="rom_data_driven_gpr_training_summary.txt")
    ...
    summary_name = str(args.summary_name).strip() or "rom_data_driven_gpr_training_summary.txt"
    summary_path = os.path.join(STAGE3_DIR, summary_name)

    print(f"[ROM-DataDriven-GPR] dataset_dir = {dataset_dir}")
    print(f"[ROM-DataDriven-GPR] dataset_root = {dataset_root} (ntot={dataset_ntot})")
    print(f"[ROM-DataDriven-GPR] solve_backend = {dataset_meta.get('solve_backend')}")
    ...
    print(f"[ROM-DataDriven-GPR] Loaded: M={n_samples}, in_dim={in_dim}, out_dim={out_dim}")
    ...
    if val_split == "mu_group":
        ...
        print(
            "[ROM-DataDriven-GPR] split = mu_group "
        ...
        print("[ROM-DataDriven-GPR] split = row (ANN-style train_test_split shuffle)")
    else:
    ...
    print(f"[ROM-DataDriven-GPR] train_used = {x_tr.shape[0]} (removed_duplicates={removed})")
    print(f"[ROM-DataDriven-GPR] val_used = {x_va.shape[0]}")
    print(f"[ROM-DataDriven-GPR] kernel_init = {kernel}")
    ...
    print(f"[ROM-DataDriven-GPR] Training done in {elapsed:.2f}s")
    print(f"[ROM-DataDriven-GPR] learned_kernel = {learned_kernel}")
    print(f"[ROM-DataDriven-GPR] val_rel_frob_percent = {va_rel_raw:.4f}%")
    print(f"[ROM-DataDriven-GPR] Saved checkpoint: {model_path}")
    print(f"[ROM-DataDriven-GPR] Summary: {summary_path}")
