...
"""Maday-isolated wrapper for Case-2 GPR trainer.

Redirects Stage-3 outputs to `Results_Maday/<tag>/Stage3/`.
"""
...
def main(argv=None):
    ...
    import stage3_perform_training_case_2_gpr as base
    ...
    if "--summary-name" not in passthrough:
        passthrough.extend(["--summary-name", f"case2_training_summary_gpr__{paths.tag}.txt"])

    print(f"[MADAY-STAGE3-GPR] tag={paths.tag}")
    print(f"[MADAY-STAGE3-GPR] stage3_dir={paths.stage3}")
    print(f"[MADAY-STAGE3-GPR] models_dir={paths.stage3_models}")
