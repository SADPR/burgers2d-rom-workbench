...
"""
Run Case 2 ANN closure with enriched residual testing and selectable backend.
...
def _load_or_build_case2_pg_ecsw_weights(
    ...
    weights_path = os.path.join(
        RUNS_ECSW_DIR,
        f"ecsw_weights_ann_case2_pg_n{n_primary}_ntot{total_modes}.npy",
    )
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Run Case 2 ANN with enriched residual testing and PROM/HPROM backend."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional override for checkpoint path (takes precedence over --model-name).",
    )
    ...
    if device == "cuda" and not torch.cuda.is_available():
        print("[Case2-PG] CUDA requested but not available. Falling back to CPU.")
        device = "cpu"
    ...
    if solve_backend == "hprom" and not use_ecsw:
        print("[Case2-PG] solve_backend='hprom' with use_ecsw=False -> falling back to PROM solve.")
        effective_backend = "prom"
    if solve_backend == "prom" and use_ecsw:
        print("[Case2-PG] use_ecsw=True ignored because solve_backend='prom'.")
    ...
    print(f"[Case2-PG] device = {device}")
    print(f"[Case2-PG] checkpoint = {model_path}")
    print(f"[Case2-PG] basis = {basis_path}")
    print(f"[Case2-PG] u_ref = {uref_path if os.path.exists(uref_path) else 'zeros'}")
    print(f"[Case2-PG] solve_backend(requested) = {solve_backend}")
    print(f"[Case2-PG] solve_backend(effective) = {effective_backend}")
    print(f"[Case2-PG] use_ecsw = {use_ecsw}")
    ...
    if effective_backend == "hprom":
        ...
        print(f"[Case2-PG] ECSW weights: {weights_path} ({weights_source})")
        print(f"[Case2-PG] ECSW training trajectories used = {ecsw_num_training_mu}")
        print(f"[Case2-PG] N_e = {n_ecsw_elements}")
        print(f"[Case2-PG] ECSW residual = {ecsw_residual}")
    ...
    run_tag = f"case2_petrov_galerkin_{backend_tag}_ann_{tag}_n{n_p}_ntot{total_modes}"
    ...
    plot_snaps(
        ...
        rom_snaps,
        plot_steps,
        label="HPROM-ANN Case 2 (PG test)" if effective_backend == "hprom" else "PROM-ANN Case 2 (PG test)",
        fig_ax=(fig, ax1, ax2),
        color="blue",
        linewidth=1.8,
        linestyle="solid",
    ...
    write_kv_txt(
        summary_txt,
        [
            ("solver_variant", "case2_petrov_galerkin_residual_testing"),
            ("mu_test", mu_test),
            ("device", device),
    ...
    print(f"[Case2-PG] ecsw_setup_elapsed = {ecsw_setup_elapsed:.3e} s")
    print(f"[Case2-PG] online_solve_elapsed = {online_solve_elapsed:.3e} s")
    print(f"[Case2-PG] elapsed = {elapsed:.3e} s")
    print(f"[Case2-PG] its={num_its} | jac={jac_time:.3e} | res={res_time:.3e} | ls={ls_time:.3e}")
    print(f"[Case2-PG] relative error vs HDM: {rel_err:.2f}%")
    print(f"[Case2-PG] saved snaps: {out_npy}")
    print(f"[Case2-PG] saved plot:  {out_png}")
    print(f"[Case2-PG] summary:     {summary_txt}")
