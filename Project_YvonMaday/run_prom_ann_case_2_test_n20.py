...
"""
Experimental runner for Case 2 ANN closure ROM with selectable backend.
...
def main(argv=None):
    ...
    parser.add_argument(
        "--model-name",
        type=str,
        default="case2_model_test_n20.pt",
    ...
    run_tag_prefix = str(args.run_tag_prefix).strip() or "case2_test"
    ...
    print(f"[Case2] arch.hidden_dims = {model_hidden_dims}")
    print(f"[Case2] arch.activation = {model_activation}")
    print(f"[Case2] arch.dropout = {model_dropout}")
    ...
    run_tag = f"{run_tag_prefix}_{backend_tag}_ann_{tag}_n{n_p}_ntot{total_modes}"
