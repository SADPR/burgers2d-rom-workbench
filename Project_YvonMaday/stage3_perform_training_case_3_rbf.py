...
"""Train Case-3 RBF closure map with grid-search hyperparameter selection.

Case 3 mapping:
    qN_s = N_rbf(qN_p, mu1, mu2, t)
"""
...
try:
    ...
    from .stage3_qn_utils import resolve_primary_modes
try:
    from stage3_perform_training_case_3_ann import load_prom_dataset_case3
except ModuleNotFoundError:
    from .stage3_perform_training_case_3_ann import load_prom_dataset_case3
try:
...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Train Case-3 RBF map qN_s=N_rbf(qN_p,mu1,mu2,t) with grid search."
    )
    ...
    parser.add_argument("--model-name", type=str, default="case3_model_rbf.pt")
    parser.add_argument("--summary-name", type=str, default="case3_training_summary_rbf.txt")
    ...
    print(f"[Case3-RBF] dataset_dir = {dataset_dir}")
    print(f"[Case3-RBF] dataset_root = {dataset_root} (ntot={dataset_ntot})")
    print(f"[Case3-RBF] solve_backend = {dataset_meta.get('solve_backend')}")
    print(f"[Case3-RBF] primary_modes = {n_primary}")
    print(f"[Case3-RBF] kernels = {kernels}")
    print(f"[Case3-RBF] epsilon_grid = {eps_grid}")
    print(f"[Case3-RBF] lambda_grid = {lam_grid}")
    print(f"[Case3-RBF] max_centers = {int(args.max_centers)}")

    x_raw, y_raw = load_prom_dataset_case3(dataset_root, primary_modes=n_primary)
    m, in_dim = x_raw.shape
    _, out_dim = y_raw.shape
    print(f"[Case3-RBF] Loaded: M={m}, in_dim={in_dim}, out_dim={out_dim}")
    ...
    ckpt = {
        "format": "rbf_map",
        "case": "case3",
        "mapping": "qN_s = N_rbf(qN_p, mu1, mu2, t)",
    ...
    print(f"[Case3-RBF] Training done in {elapsed:.2f}s")
    print(f"[Case3-RBF] Saved checkpoint: {model_path}")
    print(f"[Case3-RBF] Summary: {summary_path}")
