...
def main(
    ...
    print(f"[LOCAL-HPROM-GPR] Cluster selector mode: {selector_mode}")
    ...
    if compute_ecsw:
        ...
        print(
            "[LOCAL-HPROM-GPR] ECSW snapshot selection mode="
        ...
        for imu, mu_train in enumerate(mu_samples):
            ...
            print(
                f"[LOCAL-HPROM-GPR] mu={mu_train}: selected {int(now_cols.size)} "
        ...
        print(
            f"[LOCAL-HPROM-GPR] Selected snapshots per mu: {ecsw_plan['num_selected_per_mu']}"
        )
        ecsw_sampling_3d_plot_path = os.path.join(
            results_dir, "local_hprom_gpr_ecsw_sampling_3d.png"
        )
        save_ecsw_sampling_3d_plot(
            mu_points=mu_samples,
            dt=dt,
            ...
            title="Local HPROM-GPR ECSW Snapshot Selection in $(\\mu_1,\\mu_2,t)$",
        )
        print(
            f"[LOCAL-HPROM-GPR] 3D sampling plot saved to: {ecsw_sampling_3d_plot_path}"
        )
        ...
        print(
            f"[LOCAL-HPROM-GPR] ECSW/ECM SVD method: {ecsw_svd_method} "
